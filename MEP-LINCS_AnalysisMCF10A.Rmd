...
```{r, child='MEP-LINCS_RUV3LoessResidualsText.Rmd', eval=unique(l3$NormMethod)=="RUV3LoessResiduals" & (!cellLine=="MCF10A")}
```



```{r, child='MEP-LINCS_RUV3ResidualsText.Rmd', eval=unique(l3$NormMethod)=="RUV3LoessResiduals" & grepl("SS3",ss) & cellLine=="MCF10A"}
```
...
if(any(grepl("SS1",ss))){
  ...
  if (cellLine=="MCF10A"){
    colNames <- c("MEP", "ECMp", "Ligand", "Spot_PA_SpotCellCountLog2RUV3", "Spot_PA_SpotCellCount","Nuclei_PA_Cycle_DNA2NProportionLogitRUV3","Cytoplasm_CP_Intensity_MedianIntensity_KRT5Log2RUV3", "Spot_PA_ReplicateCount")
    colNamesDisplay <- c("MEP","ECMp","Ligand","Normed Spot Cell Count ","Spot Cell Count","Normed DNA 2N Prop.","Normed KRT5 Intensity","Replicate Count")
  }else{
...
```{r, child='MEP-LINCS_LineageTextMCF10A.Rmd',eval=grepl("SS3",ss)}
```

```{r KRT5ByMEP,eval=grepl("SS3",ss), fig.width=8, fig.height=6}

dt <- l4HF

p <- ggplot(dt, aes(x =reorder(MEP, Cytoplasm_CP_Intensity_MedianIntensity_KRT5Log2RUV3), y = Cytoplasm_CP_Intensity_MedianIntensity_KRT5Log2RUV3))+
  geom_errorbar(aes(ymin=Cytoplasm_CP_Intensity_MedianIntensity_KRT5Log2RUV3-Cytoplasm_CP_Intensity_MedianIntensity_KRT5Log2RUV3_SE, ymax=Cytoplasm_CP_Intensity_MedianIntensity_KRT5Log2RUV3+Cytoplasm_CP_Intensity_MedianIntensity_KRT5Log2RUV3_SE), width=.01, colour="black") +
  xlab("MEP")+ylab("Normalized Logged KRT5 Intensity")+
  geom_point(colour = "blue", alpha = .5)+
  theme( axis.text.x=element_blank(), axis.ticks=element_blank(),  panel.grid.major = element_blank())+
  ggtitle("MEPs Ordered by Normalized KRT5 Intensity with SE Bars")
...
```{r LineageMarkers, echo=FALSE, fig.width=8, fig.height=6,eval=grepl("SS3",ss) & FALSE}
...
```{r KRT5HeatmapFull, fig.width=8, fig.height=5, eval=grepl("SS3",ss)}

#Cast to get ECMps in rows and ligands into columns
df <- dcast(data.frame(l4F[,list(ECMp,Ligand, Cytoplasm_CP_Intensity_MedianIntensity_KRT5Log2RUV3,Barcode)]),ECMp~Ligand, value.var = "Cytoplasm_CP_Intensity_MedianIntensity_KRT5Log2RUV3")
...
```{r KRT5HeatmapHF, fig.width=8, fig.height=5, eval=grepl("SS3",ss)}

#Cast to get ECMps in rows and ligands into columns
df <- dcast(data.frame(l4HF[,list(ECMp,Ligand, Cytoplasm_CP_Intensity_MedianIntensity_KRT5Log2RUV3,Barcode)]),ECMp~Ligand, value.var = "Cytoplasm_CP_Intensity_MedianIntensity_KRT5Log2RUV3")
