...
setkey(l4,"Ligand")
p <- ggplot(dt, aes(x=Cytoplasm_CP_Intensity_MedianIntensity_MitoTrackerNorm))+geom_histogram(binwidth = .1)+
...
d3heatmap(l4pcvDT, colors=hmcols, xaxis_font_size="6pt", yaxis_font_size="4pt", show_grid = FALSE)
```
...
for (barcode in barcodes){
  ...
  if ("Nuclei_CP_Intensity_IntegratedIntensity_Caspase" %in% colnames(DT))  {    
    p <- create8WellPseudoImage(DT, pr = "Nuclei_CP_Intensity_IntegratedIntensity_Caspase", prDisplay = unique(DT$Endpoint488))
    suppressWarnings(print(p))
  }
  
  if ("Nuclei_CP_Intensity_MedianIntensity_Nestin" %in% colnames(DT))  {    
