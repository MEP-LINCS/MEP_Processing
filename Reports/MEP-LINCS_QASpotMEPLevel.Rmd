...
PCFVNames <- grep("Norm|Loess",colnames(l4),value=TRUE)
PCFVNames <- grep("Euler|MaximumRadius|_SE|_RobustZ|Method",PCFVNames,invert = TRUE, value = TRUE)

#Get the PCFVs
l4PCAModel <- prcomp(as.formula(paste(" ~ ", paste(PCFVNames, collapse = "+"))), data = l4, scale= TRUE)

#Extract the first PCs of each MEP into a matrix
l4pcvDT <- data.table(l4PCAModel$x[,1:nrPCs],Barcode =l4$Barcode, MEP = l4$MEP)
