...
x <- data.frame(cellLine="HMEC122L",
                drug= "none",
...
plotPCs(m = RUV3LoessMEPFBSPCAModel,PCs = c("PC1","PC2","PC3"),colour = "l4MergeFBS$Barcode", title = "PCA of All MEP Signals\n", legendTitle = "Barcode", xlim = c(-15,15), ylim=c(-10,10))
print(p)
...
p <- ggplot(l4MergeFBSU, aes(x=V1, y=V2, colour=Ligand, text=paste("ECMp:",ECMp)))+
  ...
  ggtitle(paste("tSNE plot of HMEC122L MEPs with FBS Coloured by Ligand"))+
  guides(colour = FALSE)
ggplotly(p)
...
p <- ggplot(l4Merge, aes(x=V1, y=V2, colour=Ligand, text=paste("ECMp:",ECMp)))+
  geom_point(size=rel(.2), alpha=.5)+
  ggtitle(paste("tSNE plot of HMEC122L MEPs Coloured by Ligand without FBS"))+
  guides(colour = FALSE)
ggplotly(p)
...
p <- ggplot(l4Merge, aes(x=V1, y=V2, colour=ECMp, text=paste("Ligand:",Ligand)))+
  geom_point(size=rel(.2), alpha=.5)+
  ggtitle(paste("tSNE plot of HMEC122L MEPs Coloured by ECM Protein"))+
  guides(colour =FALSE)
ggplotly(p)
...
p <- ggplot(l4Merge, aes(x=V1, y=V2, colour=Barcode, text=paste("Ligand:",Ligand,"ECMp:",ECMp)))+
  geom_point(size=rel(.2), alpha=.5)+
  ggtitle(paste("tSNE plot of HMEC122L MEPs Coloured by Barcode"))+
  guides(colour =FALSE)
ggplotly(p)
...
p <- ggplot(l4Merge, aes(x=V1, y=V2, colour=Proliferation, text=paste("Ligand:",Ligand,"ECMp:",ECMp)))+
  geom_point(size=rel(.2), alpha=.5)+
  ggtitle(paste("tSNE plot of HMEC122L MEPs Coloured by Ligand Effect"))+
  guides(colour =FALSE)
ggplotly(p)
...
write.csv(nomMEPSCurated,file = "HMEC122LMEPSforValidation.csv",row.names = FALSE)
