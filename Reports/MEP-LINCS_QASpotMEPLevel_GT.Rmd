...
barcodes <- c("LI8X01121", "LI8X01122", "LI8X01123", "LI8X01124","LI8X01125", "LI8X01126", "LI8X01127", "LI8X01128", "LI8X01129")

l2 <- lapply(barcodes, function(barcode, path){
  sd <- read_csv(paste0(path,"/",barcode,"/Analysis/GT1/",barcode,"_level_2.csv"))
}, path=path) %>%
  bind_rows()
...
Boxplots of the spot cell counts show the values stratified by plate, well and ligand. The blue lines show the dataset median. Each boxplot summarizes data from ~99 COL1 spots.     

```{r SCCBoxplots, fig.height=5}
df <- l2 %>%
  filter(str_detect(ECMp,"COL1_"))

p <- ggplot(df, aes(x=factor(Barcode_Well_Ligand), y=Spot_PA_SpotCellCount, colour=Well))+
...
p <- p + geom_hline(yintercept = median(df$Spot_PA_SpotCellCount), colour="blue", alpha=.5)
print(p)
...
p <- ggplot(df, aes(x=reorder(factor(Ligand),X = Spot_PA_SpotCellCount, FUN = median), y=Spot_PA_SpotCellCount, colour=factor(Barcode)))+
  geom_boxplot(outlier.size = .5)+
...
ECM boxplots summarize the spotted ECM proteins paired with PBS in each plate.  


```{r SCCBoxplotsbyECMp, fig.height=5}
df <- l2 %>%
  filter(Ligand == "PBS")

p <- ggplot(df, aes(x=reorder(factor(ECMp),X = Spot_PA_SpotCellCount, FUN = median), y=Spot_PA_SpotCellCount))+
  geom_boxplot(outlier.size = .5)+
  labs(x="ECMp",y="Spot Cell Count", title="Spot Cell Count by ECMp and Barcode")+
  theme(axis.text.x = element_text(angle = 90, vjust = 0, hjust=0.5, size=rel(1)), axis.text.y = element_text(angle = 0, vjust = 0.5, hjust=1, size=rel(1)), plot.title = element_text(size = rel(1)),legend.text=element_text(size = rel(.3)),legend.title=element_text(size = rel(.8)))
p <- p + geom_hline(yintercept = median(df$Spot_PA_SpotCellCount), colour="blue", alpha=.5)
print(p)
...
p <- ggplot(l2, aes(x=factor(Barcode), y=log2(Nuclei_GT_DAPI_MeanIntensity), colour=Well))+
  geom_boxplot(outlier.size = .5)+
...
p <- p + geom_hline(yintercept = log2(median(l2$Nuclei_GT_DAPI_MeanIntensity)), colour="blue", alpha=.5)
print(p)

```


```{r, child='MEP-LINCS_EdUText.Rmd', eval="Nuclei_GT_EdU_MeanIntensity" %in% colnames(l2)}
```



```{r rawEdUBoxplotsPlate, fig.height=5,  eval="Nuclei_GT_EdU_MeanIntensity" %in% colnames(l2)}

p <- ggplot(l2, aes(x=factor(Barcode), y=log2(Nuclei_GT_EdU_MeanIntensity), colour=Well))+
  geom_boxplot(outlier.size = .5)+
  labs(x="Barcode",y="Median Intensity EdU (log2)",colour="Well", title="Median EdU Intensity per Spot by Well and Barcode")+
  theme(axis.text.x = element_text(angle = 90, vjust = 0, hjust=0.5, size=rel(1)), axis.text.y = element_text(angle = 0, vjust = 0.5, hjust=1, size=rel(1)), plot.title = element_text(size = rel(1)),legend.text=element_text(size = rel(.3)),legend.title=element_text(size = rel(.8)))
p <- p + geom_hline(yintercept = log2(median(l2$Nuclei_GT_EdU_MeanIntensity)), colour="blue", alpha=.5)
print(p)

```


```{r rawEdUBoxplots, fig.height=5, eval="Nuclei_PA_Gated_EdUPositiveProportion" %in% colnames(l2)}

df <- l2 %>%
  filter(str_detect(ECMp, "COL1_"))

p <- ggplot(df, aes(x=reorder(factor(Ligand), X = Nuclei_PA_Gated_EdUPositiveProportion, FUN = median), y=Nuclei_PA_Gated_EdUPositiveProportion, colour=factor(Barcode)))+
  geom_boxplot(outlier.size = .5)+
  coord_cartesian(y=c(0,1))+
  labs(x="Ligand",y="EdU+ Proportion", colour="Barcode", title="EdU+ Proportion per Spot by Ligand and Barcode")+
  theme(axis.text.x = element_text(angle = 90, vjust = 0, hjust=0.5, size=rel(1)), axis.text.y = element_text(angle = 0, vjust = 0.5, hjust=1, size=rel(1)), plot.title = element_text(size = rel(1)),legend.text=element_text(size = rel(.3)),legend.title=element_text(size = rel(.8)))
p <- p + geom_hline(yintercept = median(df$Nuclei_PA_Gated_EdUPositiveProportion), colour="blue", alpha=.5)
print(p)

df <- l2 %>%
  filter(str_detect(Ligand, "PBS"))

p <- ggplot(df, aes(x=reorder(factor(ECMp), X = Nuclei_PA_Gated_EdUPositiveProportion, FUN = median), y=Nuclei_PA_Gated_EdUPositiveProportion))+
  geom_boxplot(outlier.size = .5)+
  coord_cartesian(y=c(0,1))+
  labs(x="ECMp",y="EdU+ Proportion", colour="Barcode", title="EdU+ Proportion per Spot by ECMp")+
  theme(axis.text.x = element_text(angle = 90, vjust = 0, hjust=0.5, size=rel(1)), axis.text.y = element_text(angle = 0, vjust = 0.5, hjust=1, size=rel(1)), plot.title = element_text(size = rel(1)),legend.text=element_text(size = rel(.3)),legend.title=element_text(size = rel(.8)))
p <- p + geom_hline(yintercept = median(df$Nuclei_PA_Gated_EdUPositiveProportion), colour="blue", alpha=.5)
print(p)
```
...
p <- ggplot(l2, aes(x=Cytoplasm_PA_Intensity_LineageRatio))+geom_histogram(binwidth = .02)+
...
```{r LineageRatioByLigand, eval=any(grepl("Cytoplasm_PA_Intensity_LineageRatio",colnames(l2)))}

df <- l2 %>%
  filter(str_detect(ECMp, "COL1_"))
p <- ggplot(df, aes(x = reorder(Ligand, Cytoplasm_PA_Intensity_LineageRatio, FUN=median), y = Cytoplasm_PA_Intensity_LineageRatio, colour = Barcode))+geom_boxplot()+
...
df <- l2 %>%
  filter(str_detect(Ligand, "PBS"))
p <- ggplot(df, aes(x = reorder(ECMp, Cytoplasm_PA_Intensity_LineageRatio, FUN=median), y = Cytoplasm_PA_Intensity_LineageRatio))+geom_boxplot()+
...
```{r KRT5ByECMpl2, eval=any(grepl("Cytoplasm_GT_KRT5_MeanIntensity",colnames(l2)))}

df <- l2 %>%
  filter(str_detect(Ligand, "PBS"))
p <- ggplot(df, aes(x = reorder(ECMp, Cytoplasm_GT_KRT5_MeanIntensity, FUN=median), y = Cytoplasm_GT_KRT5_MeanIntensity))+geom_boxplot()+
...
p <- ggplot(df, aes(x = reorder(ECMp, Cytoplasm_GT_KRT19_MeanIntensity, FUN=median), y = Cytoplasm_GT_KRT19_MeanIntensity))+geom_boxplot()+
  ggtitle(paste("MEP  KRT19 by ECM Protein"))+
  xlab("")+ylab("")+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size=rel(.8)), axis.text.y = element_text(angle = 0, vjust = 0.5, hjust=1, size=rel(1)), plot.title = element_text(size = rel(1)),legend.text=element_text(size = rel(.3)),legend.title=element_text(size = rel(.3)))
suppressWarnings(print(p))
...
reps <- table(l2$ECMp)/length(unique(paste(l2$Barcode, l2$Well)))
df  <- data_frame(ECMp = names(reps),
                  value = reps)

p <- ggplot(df, aes(x=ECMp, y = value))+
  geom_col(width=.8)+
  geom_hline(yintercept = median(df$value), colour="blue")+
  scale_y_log10()+
  ggtitle(" \n\nAverage count of replicate ECM proteins in each MEMA")+
...
for(barcode in barcodes){
  ...
  p <- create8WellPseudoImage(dtb, pr = "Nuclei_GT_DAPI_MeanIntensity", prDisplay = "Median DAPI")
  suppressWarnings(print(p))
  
 
  if (any(grepl("EdU",colnames(dtb))))  {    
    p <- create8WellPseudoImage(dtb, pr = "Nuclei_GT_EdU_MeanIntensity",prDisplay = "EdU")
    suppressWarnings(print(p))
  } 
  
  if (any(grepl("KRT5",colnames(dtb))))  {
    
    p <- create8WellPseudoImage(dtb, pr = "Cytoplasm_GT_KRT5_MeanIntensity", prDisplay = "KRT5")
    suppressWarnings(print(p))
  }
  
  if (any(grepl("KRT19",colnames(dtb))))  {
    
    p <- create8WellPseudoImage(dtb, pr = "Cytoplasm_GT_KRT19_MeanIntensity", prDisplay = "KRT19")
    suppressWarnings(print(p))
  }
}
```
