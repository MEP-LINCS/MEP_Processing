...
```{r, child='MEP-LINCS_PCAText.Rmd', eval=!any(duplicated(l2$MEP_Drug))}
```

<br>

```{r createPCAModel, eval=!any(duplicated(l2$MEP_Drug))}
...
PCFVNames <- grep("|Loess",colnames(dt),value=TRUE)
...
```{r PCAHeatmap, fig.height=8, fig.width=7,eval=!any(duplicated(l2$MEP_Drug))}
