...
l1DTL <- lapply(barcodes, function(barcode, path){
  sd <- fread(paste0(path,barcode,"/Analysis/GT1/",barcode,"_level1.csv"),verbose = FALSE, showProgress = FALSE)
  set.seed(42)
...
if("TotalDNANormed" %in% colnames(l1S)){
  ...
  xaxisMax <- quantile(l1S$Nuclei_CP_Intensity_IntegratedIntensity_Dapi,probs=.995)
}
...
for (barcode in barcodes){
  setkey(l1S,Barcode)
  mDT <- l1S[barcode]
    if(!anyNA(mDT$Nuclei_CP_Intensity_MedianIntensity_EdU)){
...
for (barcode in barcodes){
  mDT <- l1S[l1S$Barcode == barcode]
    #Squish the extreme values for better display
  mDT <- mDT[mDT$Cytoplasm_CP_Intensity_MedianIntensity_KRT14 > quantile(mDT$Cytoplasm_CP_Intensity_MedianIntensity_KRT14, probs=.01, na.rm=TRUE) & mDT$Cytoplasm_CP_Intensity_MedianIntensity_KRT14 < quantile(mDT$Cytoplasm_CP_Intensity_MedianIntensity_KRT14,probs=.98, na.rm=TRUE),]
  p <- ggplot(mDT, aes(x=log2(Cytoplasm_CP_Intensity_MedianIntensity_KRT14)))+
    geom_histogram(binwidth = .01)+
    facet_wrap(~Well_Ligand, nrow=2)+
    ggtitle(paste("KRT14 Intensity,",barcode))+
...
```{r rawVimentinHistograms, fig.width=3.7,fig.height=4, eval="Cytoplasm_CP_Intensity_MedianIntensity_Vimentin" %in% colnames(l1S)}
for (barcode in barcodes){
  mDT <- l1S[l1S$Barcode == barcode]
  #Squish the extreme values for better display
  mDT <- mDT[mDT$Cytoplasm_CP_Intensity_MedianIntensity_Vimentin > quantile(mDT$Cytoplasm_CP_Intensity_MedianIntensity_Vimentin, probs=.01, na.rm=TRUE) & mDT$Cytoplasm_CP_Intensity_MedianIntensity_Vimentin < quantile(mDT$Cytoplasm_CP_Intensity_MedianIntensity_Vimentin,probs=.98, na.rm=TRUE),]
  p <- ggplot(mDT, aes(x=log2(Cytoplasm_CP_Intensity_MedianIntensity_Vimentin)))+
    geom_histogram(binwidth = .01)+
    facet_wrap(~Well_Ligand, nrow=2)+
    ggtitle(paste("Vimentin Intensity,",barcode))+
    ylab("Count")+xlab("Median Intensity Vimentin\nLog2")+
    theme(axis.text.x = element_text(angle = 90, vjust = 0, hjust=0.5, size=rel(1)), axis.text.y = element_text(angle = 0, vjust = 0.5, hjust=1, size=rel(1)), plot.title = element_text(size = rel(1)),legend.text=element_text(size = rel(.3)),legend.title=element_text(size = rel(.3)), strip.text = element_text(size = rel(.5)))
  suppressWarnings(print(p))
}
