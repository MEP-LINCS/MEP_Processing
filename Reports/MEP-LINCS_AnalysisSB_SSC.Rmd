...
#Debug new function mode
addOmeroIDs <- function(dt){
  if(any(grepl("^ClarionID$",colnames(dt)))){
    dt <- dt[,OmeroDetailURL := paste0('<a href="https://www.lincsclarion.org/webclient/img_detail/',ClarionID,'/"',' target="_blank">Omero</a>')]
    dt <- dt[,OmeroThumbnailURL := paste0('<a href="https://www.lincsclarion.org/webclient/render_thumbnail/',ClarionID,'/"',' target="_blank">Omero</a>')]
    dt <- dt[,OmeroImageURL := paste0('<a href="https://www.lincsclarion.org/webclient/render_image/',ClarionID,'/"',' target="_blank">Omero</a>')]
  } else if(any(grepl("^ImageID$",colnames(dt)))){
...
###The Spot Level Interactive Datatable can be filtered on keywords and sorted by values.Images can be viewed by clicking on the Image Link (Gray lab Omero account needed).  
...
NormPCAModel <- createPCAModel(dt=l4C, fvNames = NormNames)
...
plotLEHmap(l4C, hclustfun = "hclust", fill="Spot_PA_SpotCellCountNorm", titleExpression=titleExpression, limits=limits)
