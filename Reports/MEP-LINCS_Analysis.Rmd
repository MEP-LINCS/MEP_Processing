...
p <- ggplot(dt, aes(x =reorder(MEP, Spot_PA_SpotCellCountNorm), y = Spot_PA_SpotCellCountNorm))+
  ...
  coord_cartesian(ylim = c(0,1.25*max(dt$Spot_PA_SpotCellCountNorm)))+
...
```{r DNA2NByMEPFull, fig.width=8, fig.height=6, eval="Nuclei_PA_Cycle_DNA2NProportion" %in% colnames(l4F)}
...
```{r DNA2NHeatmapFull, fig.width=8, fig.height=5, eval="Nuclei_PA_Cycle_DNA2NProportion" %in% colnames(l4F)}
...
```{r LineageMarkers, echo=FALSE, fig.width=8, fig.height=6,eval=("Cytoplasm_CP_Intensity_MedianIntensity_KRT5Norm" %in% colnames(l4F))& ("Cytoplasm_CP_Intensity_MedianIntensity_KRT19Norm" %in% colnames(l4F))}
...
```{r LineagRatioHeatmapFull, fig.width=8, fig.height=5, eval=("Cytoplasm_PA_Intensity_LineageRatioNorm" %in% colnames(l4F))}
...
try(d3heatmap(dfZoom(df, 0.03, .97), colors=hmcols, xaxis_font_size="6pt", yaxis_font_size="5pt"), TRUE)

```


```{r EdUvsCellCount, echo=FALSE, fig.width=8, fig.height=6, eval=("Spot_PA_SpotCellCountNorm" %in% colnames(l4)) & ("Nuclei_PA_Gated_EdUPositiveProportionNorm" %in% colnames(l4))}

dt <- l4

p <- ggplot(dt, aes(x=Spot_PA_SpotCellCountNorm, y = Nuclei_PA_Gated_EdUPositiveProportionNorm, colour = Ligand))+
  geom_point(alpha=.5)+
  guides(colour=FALSE)+
  xlab("Spot Cell Count")+ylab("EdU Proliferation")+
  ggtitle("EdU vs Cell Count by Ligand")
(gg <- ggplotly(p))
```



```{r EdUvsLineageMarkers, echo=FALSE, fig.width=8, fig.height=6, eval=("Cytoplasm_PA_Intensity_LineageRatioNorm" %in% colnames(l4)) & ("Nuclei_PA_Gated_EdUPositiveProportionNorm" %in% colnames(l4))}
...
tSNEParameters <-grep("Norm$",colnames(l4), value=TRUE)
data_matrix <- scale(as.matrix(l4[,tSNEParameters, with=FALSE]))
data_matrix <- data_matrix[,!apply(data_matrix,2,anyNA)]
set.seed(1234)
...
```{r ggplotlytSNEMEPs,fig.width=12, fig.height=6, eval=TRUE}
