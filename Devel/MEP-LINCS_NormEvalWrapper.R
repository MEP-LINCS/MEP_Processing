...
PC3df <- data.frame(cellLine=rep(c("PC3"), 4),
                    ...
                    analysisVersion=c("av1.4","av1.4","av1.4","av1.5"),
...
x <- ssDatasets[c(4),]
renderNormEvalReports <- function(x){
  cellLine <- x[["cellLine"]]
  ss <- x[["ss"]]
  rawDataVersion <- x[["rawDataVersion"]]
  analysisVersion <- x[["analysisVersion"]]
  render("MEP-LINCS_NormEval.Rmd",
         output_file = paste0("./NormEvalReports/MEP-LINCS_NormEval_",
                              x[["cellLine"]],"_",
                              x[["ss"]],"_",
                              x[["rawDataVersion"]],"_",
...
tmp <- apply(ssDatasets[c(4),], 1, renderNormEvalReports)
