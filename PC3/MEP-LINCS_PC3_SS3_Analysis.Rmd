...
p <- ggplot(data.frame(l4PCAModel$x), aes(x = PC1, y = PC2, col = DT$Cytoplasm_CP_Intensity_IntegratedIntensity_KRT19_RZSNorm)) +
  geom_point(size = rel(.8), alpha = .8) +
...
p <- ggplot(data.frame(l4PCAModel$x), aes(x = PC2, y = PC3, col = DT$Cytoplasm_PA_Intensity_LineageRatio_RZSNorm)) +
  geom_point(size = rel(.8), alpha = .8) +
...
p <- ggplot(data.frame(l4PCAModel$x), aes(x = PC1, y = PC2, col = DT$Cells_CP_AreaShape_Perimeter_RZSNorm)) +
  geom_point(size = rel(.8), alpha = .8) +
  labs(colour = "Perimeter Proportion Status")+
  theme(legend.text=element_text(size = 6))+
  guides(colour = guide_legend(override.aes = list(size=6)))
print(p)
