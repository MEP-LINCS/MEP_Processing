...
```{r LineageMarkers, echo=FALSE, eval=TRUE}
...
The lineage ratio analysis is run using normalized data.

```{r lineageRatioHistogram, eval=TRUE}
...
```{r LineageRatioByLigand, eval=TRUE}
...
```{r lineageRatioByECMp, eval=TRUE}
...
###Active MEPS from Lineage Ratio Analysis
The table below shows the first and last MEPs ranked by their lineage ratio.

```{r LineageRatioMEPS, echo = FALSE, eval=TRUE}
...
p <- ggplot(data.frame(l4PCAModel$x), aes(x = PC1, y = PC2, col = DT$Cytoplasm_PA_Intensity_LineageRatio_RZSNorm)) +
  geom_point(size = rel(.8), alpha = .8) +
...
p <- ggplot(data.frame(l4PCAModel$x), aes(x = PC3, y = PC4, col = DT$Cytoplasm_PA_Intensity_LineageRatio_RZSNorm)) +
  geom_point(size = rel(.8), alpha = .8) +
...
p <- ggplot(data.frame(l4PCAModel$x), aes(x = PC5, y = PC6, col = DT$Cytoplasm_PA_Intensity_LineageRatio_RZSNorm)) +
  geom_point(size = rel(.8), alpha = .8) +
  labs(colour = "Lineage Ratio")+
  theme(legend.text=element_text(size = 6))+
  guides(colour = guide_legend(override.aes = list(size=6)))
...
p <- ggplot(data.frame(l4PCAModel$x), aes(x = PC7, y = PC8, col = DT$Cytoplasm_PA_Intensity_LineageRatio_RZSNorm)) +
  geom_point(size = rel(.8), alpha = .8) +
  labs(colour = "Lineage Ratio")+
  theme(legend.text=element_text(size = 6))+
  guides(colour = guide_legend(override.aes = list(size=6)))
