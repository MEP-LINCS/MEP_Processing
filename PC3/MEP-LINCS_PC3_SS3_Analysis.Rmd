...
activeMEPs <- ubHeatmap(fvDT[!grepl("NID1",fvDT$MEP)], title = "MEP Feature Vectors", cols = plateCol, activeThresh = 0 )
...
p <- ggplot(data.frame(l4PCAModel$x), aes(x = PC1, y = PC2, col = l4F$Cytoplasm_CP_Intensity_IntegratedIntensity_KRT19_RZSNorm)) +
  geom_point(size = rel(.8), alpha = .8) +
...
p <- ggplot(data.frame(l4PCAModel$x), aes(x = PC2, y = PC3, col = l4F$Cytoplasm_PA_Intensity_LineageRatio_RZSNorm)) +
  geom_point(size = rel(.8), alpha = .8) +
