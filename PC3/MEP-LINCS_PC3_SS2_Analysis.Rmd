...
```{r Heatmaps_QAScores, echo=FALSE, fig.width=3.7,fig.height=4, eval=FALSE}
...
```{r ProliferationResponseLigand, fig.width=8,  fig.height=6, echo=FALSE, eval=TRUE}
...
```{r ProliferationResponseECMp, fig.width=8,  fig.height=6, echo=FALSE, eval=TRUE}
...
```{r ProliferationResponseMEP, eval=TRUE}
...
###Active MEPS from the Proliferation Analysis
The table below shows the first and last MEPs ranked by their EdU Proliferation Proportion.

```{r ProliferationMEPS, echo = FALSE, eval=TRUE}
...
p <- ggplot(data.frame(l4PCAModel$x), aes(x = PC1, y = PC2, col = DT$Nuclei_PA_Gated_EduPositiveProportion_RZSNorm)) +
  geom_point(size = rel(.8), alpha = .8) +
...
p <- ggplot(data.frame(l4PCAModel$x), aes(x = PC3, y = PC4, col = DT$Nuclei_PA_Gated_EduPositiveProportion_RZSNorm)) +
  geom_point(size = rel(.8), alpha = .8) +
  labs(colour = "Median EdU")+
  theme(legend.text=element_text(size = 6))+
  guides(colour = guide_legend(override.aes = list(size=6)))
...
p <- ggplot(data.frame(l4PCAModel$x), aes(x = PC5, y = PC6, col = DT$Nuclei_PA_Gated_EduPositiveProportion_RZSNorm)) +
  geom_point(size = rel(.8), alpha = .8) +
  labs(colour = "Median EdU")+
  theme(legend.text=element_text(size = 6))+
  guides(colour = guide_legend(override.aes = list(size=6)))
...
p <- ggplot(data.frame(l4PCAModel$x), aes(x = PC7, y = PC8, col = DT$Nuclei_PA_Gated_EduPositiveProportion_RZSNorm)) +
  geom_point(size = rel(.8), alpha = .8) +
  labs(colour = "Median EdU")+
  theme(legend.text=element_text(size = 6))+
  guides(colour = guide_legend(override.aes = list(size=6)))
...
```{r Content Layout,echo=FALSE, message=FALSE, warnings=FALSE, fig.width=6, eval=FALSE}
...
```{r Layout Replicate Count,echo=FALSE, message=FALSE, warnings=FALSE, fig.width=6.5, fig.height=3, eval=FALSE}
...
```{r Loess_Model_explanation , echo=FALSE, fig.width=2.5,fig.height=4, eval=FALSE}
...
```{r FilteredReplicateCount,echo=FALSE, message=FALSE, warnings=FALSE, fig.width=6.5, fig.height=5, eval=FALSE}
