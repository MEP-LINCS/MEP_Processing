...
fullBio <- fullBio[,grep("[.](1|2)|Cells|Cytoplasm.*Dapi|Cytoplasm.*Area|CellMask|Center|Nuclei.*KRT|_SE", colnames(fullBio), value = TRUE, invert = TRUE), with = FALSE]
...
p <- ggplot(PC3DT, aes(x =reorder(MEP, Cytoplasm_PA_Intensity_LineageRatio_RZSNorm), y = Cytoplasm_PA_Intensity_LineageRatio_RZSNorm))+geom_point(size = 3)+
...
PC3DTExtremes <- PC3DT[PC3DT$Spot_PA_SpotCellCount_RZSNorm > -2]

PC3DTExtremes <- PC3DTExtremes[PC3DTExtremes$Cytoplasm_PA_Intensity_LineageRatio_RZSNorm > quantile(PC3DTExtremes$Cytoplasm_PA_Intensity_LineageRatio_RZSNorm,.995) | PC3DTExtremes$Cytoplasm_PA_Intensity_LineageRatio_RZSNorm < quantile(PC3DTExtremes$Cytoplasm_PA_Intensity_LineageRatio_RZSNorm,.005)]

setkey(PC3DTExtremes,Cytoplasm_PA_Intensity_LineageRatio_RZSNorm)

kable(data.frame(PC3DTExtremes$MEP,PC3DTExtremes$Cytoplasm_PA_Intensity_LineageRatio_RZSNorm), col.names = c("MEP","Keratin Lineage Ratio"), digits = 2, align = c("l"))

#unique(PC3DTExtremes[PC3DTExtremes$MEP=="DSG2_LYVE1",list(Barcode,Well,ArrayRow,ArrayColumn)])


p <- ggplot(PC3DT, aes(x =reorder(MEP, Nuclei_PA_Gated_EduPositiveProportion_RZSNorm), y = Nuclei_PA_Gated_EduPositiveProportion_RZSNorm))+geom_point(size = 3)+
xlab("MEP")+ylab("Normalized EdU Positive Proportion")+
  theme( axis.text.x=element_blank(), axis.ticks=element_blank(),  panel.grid.major = element_blank())+
  ggtitle("MEPs Ordered by EdU Positive Proportion")
print(p)

kable(data.frame(PC3DT$MEP[PC3DT$Nuclei_PA_Gated_EduPositiveProportion_RZSNorm > -1.5], PC3DT$Nuclei_PA_Gated_EduPositiveProportion_RZSNorm[PC3DT$Nuclei_PA_Gated_EduPositiveProportion_RZSNorm > -1.5]), col.names = c("MEP","Edu Positive Proportion"), digits = 2, align = c("l"))
