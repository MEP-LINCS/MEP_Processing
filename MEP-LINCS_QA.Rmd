...
```{r rawDAPIPlots, fig.width=3.7,fig.height=4 eval=TRUE}
...
##Spot Cell Count QA
The spot cell count QA uses  The normalized spot cell counts are summarized by the median of their replicates. The median and mad of the distribution of normalized and summarized values are used to convert to robust Z scores and are shown below. Below the distribution plot are plots with Z scores stratified by ligand and ECM protein. 


```{r SCC_responseFull, fig.width=8,  fig.height=6, echo=FALSE, eval=TRUE}
...
p <- ggplot(dt, aes(x = reorder(ECMpAnnotID, Cytoplasm_CP_Intensity_MedianIntensity_MitoTrackerRUV3Loess, FUN=median), y = Cytoplasm_CP_Intensity_MedianIntensity_MitoTrackerRUV3Loess))+geom_boxplot()+
...
p <- ggplot(dt, aes(x = reorder(ECMpAnnotID, Nuclei_PA_Gated_EduPositiveLogitRUV3Loess, FUN=median), y = Nuclei_PA_Gated_EduPositiveLogitRUV3Loess))+geom_boxplot()+
...
p <- ggplot(dt, aes(x = reorder(ECMpAnnotID, Cytoplasm_PA_Intensity_LineageRatioLog2RUV3Loess, FUN=median), y = Cytoplasm_PA_Intensity_LineageRatioLog2RUV3Loess))+geom_boxplot()+
...
p <- ggplot(dt, aes(x=Nuclei_PA_Cycle_DNA2NProportionLogitRUV3Loess))+geom_histogram(binwidth = .08, alpha=.8)+
  ggtitle(paste("\n\n","MEP DNA 2N Proportion Distribution (Normalized, full dataset"))+
...
p <- ggplot(dt, aes(x = reorder(Ligand, Nuclei_PA_Cycle_DNA2NProportionLogitRUV3Loess, FUN=median), y = Nuclei_PA_Cycle_DNA2NProportionLogitRUV3Loess, colour = Barcode))+geom_boxplot()+
  ggtitle(paste("\n\nMEP DNA 2N Proportion by Ligand (Normalized, full dataset)"))+
  xlab("")+ylab("")+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size=rel(.8)), axis.text.y = element_text(angle = 0, vjust = 0.5, hjust=1, size=rel(1)), plot.title = element_text(size = rel(1)),legend.text=element_text(size = rel(.3)),legend.title=element_text(size = rel(.3)))
suppressWarnings(print(p))
...
p <- ggplot(dt, aes(x = reorder(ECMpAnnotID, Nuclei_PA_Cycle_DNA2NProportionLogitRUV3Loess, FUN=median), y = Nuclei_PA_Cycle_DNA2NProportionLogitRUV3Loess))+geom_boxplot()+
  ggtitle(paste("\n\nMEP DNA 2N Proportion by ECM Protein (Normalized, full dataset)"))+
  xlab("")+ylab("")+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size=rel(.8)), axis.text.y = element_text(angle = 0, vjust = 0.5, hjust=1, size=rel(1)), plot.title = element_text(size = rel(1)),legend.text=element_text(size = rel(.3)),legend.title=element_text(size = rel(.3)))
suppressWarnings(print(p))
...
p <- ggplot(dt, aes(x=Nuclei_PA_Cycle_DNA2NProportionLogitRUV3Loess))+geom_histogram(binwidth = .08, alpha=.8)+
  ggtitle(paste("\n\n","MEP DNA 2N Proportion Distribution (Normalized, HF dataset"))+
  ylab("Count")+xlab("DNA 2N Proportion")+
  theme(strip.text = element_text(size = 5))
suppressWarnings(print(p))
```
...
p <- ggplot(dt, aes(x = reorder(Ligand, Nuclei_PA_Cycle_DNA2NProportionLogitRUV3Loess, FUN=median), y = Nuclei_PA_Cycle_DNA2NProportionLogitRUV3Loess, colour = Barcode))+geom_boxplot()+
  ggtitle(paste("\n\nMEP DNA 2N Proportion by Ligand (Normalized, HF dataset)"))+
  xlab("")+ylab("")+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size=rel(.8)), axis.text.y = element_text(angle = 0, vjust = 0.5, hjust=1, size=rel(1)), plot.title = element_text(size = rel(1)),legend.text=element_text(size = rel(.3)),legend.title=element_text(size = rel(.3)))
suppressWarnings(print(p))
...
p <- ggplot(dt, aes(x = reorder(ECMpAnnotID, Nuclei_PA_Cycle_DNA2NProportionLogitRUV3Loess, FUN=median), y = Nuclei_PA_Cycle_DNA2NProportionLogitRUV3Loess))+geom_boxplot()+
  ggtitle(paste("\n\nMEP DNA 2N Proportion by ECM Protein (normalized, HF dataset)"))+
  xlab("")+ylab("")+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size=rel(.8)), axis.text.y = element_text(angle = 0, vjust = 0.5, hjust=1, size=rel(1)), plot.title = element_text(size = rel(1)),legend.text=element_text(size = rel(.3)),legend.title=element_text(size = rel(.3)))
suppressWarnings(print(p))
...
PCFVNames <- grep("Norm|RUV3",colnames(l4F),value=TRUE)
