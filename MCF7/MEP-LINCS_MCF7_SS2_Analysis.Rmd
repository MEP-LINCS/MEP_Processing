...
fullBio <- fullBio[,grep("[.](1|2)|Cells|Cytoplasm.*Dapi|Cytoplasm.*Area|CellMask|Center|Nuclei.*KRT|_SE|Euler|Logit|2N|4N", colnames(fullBio), value = TRUE, invert = TRUE), with = FALSE]
...
PCFVNames <- grep("Euler|MaximumRadius|_SE|_RobustZ|Center|Logit",PCFVNames,invert = TRUE, value = TRUE)
...
m <- as.matrix(l4pcvDT[,grep("Ligand|ECMp|MEP|Barcode|2N|4N",x = colnames(l4pcvDT), value = TRUE, invert = TRUE), with = FALSE])
...
p <- ggplot(data.frame(l4PCAModel$x), aes(x = PC1, y = PC2, col = DT$Nuclei_PA_Gated_EduPositiveProportion_RZSNorm)) +
  geom_point(size = rel(.8), alpha = .8) +
  labs(colour = "EdU Positive Proportion")+
  theme(legend.text=element_text(size = 6))+
  guides(colour = guide_legend(override.aes = list(size=6)))
print(p)
