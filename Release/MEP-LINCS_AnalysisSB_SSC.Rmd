...
l3C <- l3C[,Nuclei_PA_Cycle_DNA4NProportionLogitRUV3LoessBacktransformed:= btLogit(Nuclei_PA_Cycle_DNA4NProportionLogitRUV3Loess)]
l3C <- l3C[,Nuclei_PA_Gated_EdUPositiveProportionLogitRUV3LoessBacktransformed:= btLogit(Nuclei_PA_Gated_EdUPositiveProportionLogitRUV3Loess)]
...
#Add backtransformed values for some logit transformed signals
l4C <- l4C[,Nuclei_PA_Cycle_DNA4NProportionLogitRUV3LoessBacktransformed:= btLogit(Nuclei_PA_Cycle_DNA4NProportionLogitRUV3Loess)]
l4C <- l4C[,Nuclei_PA_Gated_EdUPositiveProportionLogitRUV3LoessBacktransformed:= btLogit(Nuclei_PA_Gated_EdUPositiveProportionLogitRUV3Loess)]
...
***
This data is RUV and loess normalized and then summarized across all staining sets where each ligand has two replicates and there are 16 control wells. The NID1 and ELN data are filtered due to low cell counts that create extreme values.  
...
***
This data is RUV and loess normalized and then summarized across all staining sets where each ligand has three replicates and there are 24 control wells. The NID1 and ELN data are filtered due to low cell counts that create extreme values.  
...
###Spot Level Data.   The interactive datatable of the spot level data displays spots filtered on keywords and sorted by values. There is one image of each spot which is displayed by clicking on the Omero link (OHSU Gray Lab Omero account needed).  
...
***
This scatter plot shows the normalized KRT19+ proportion vs the normalized EdU+ measured proliferation. The spot size represents the relative number of cells. The colors represent the ligands. The ECM protein is displayed in the tool tip box that appears while hovering over a spot.  
...
rawFVNames <- grep("Log2|Logit", grep("ECMp|Ligand|MEP|Barcode|Well|StainingSet|Loess|RUV3",colnames(l4C),invert = TRUE, value = TRUE), value=TRUE)
rawPCAModel <- createPCAModel(dt=l4C, fvNames = rawFVNames)

#Get a PCA model from the RUV3 data
RUV3Names <- grep("Loess",grep("Log2RUV3$|LogitRUV3$",colnames(l4C), value=TRUE),value=TRUE,invert=TRUE)
RUV3PCAModel <- createPCAModel(dt=l4C, fvNames = RUV3Names)

#Get a PCA model from the RUV3Loess data
RUV3LoessNames <- grep("LoessRUV3",grep("Log2RUV3Loess$|LogitRUV3Loess$",colnames(l4C), value=TRUE), value=TRUE, invert=TRUE)
RUV3LoessPCAModel <- createPCAModel(dt=l4C, fvNames = RUV3LoessNames)

#Get a PCA model from the RUV3Loess of all signals
RUV3LoessMEPNames <- grep("LoessRUV3",grep("Log2RUV3Loess$|LogitRUV3Loess$",colnames(l4C), value=TRUE), value=TRUE, invert=TRUE)
RUV3LoessMEPPCAModel <- createPCAModel(dt=l4C, fvNames = RUV3LoessMEPNames)

RUV3LoessMEPFBSPCAModel <- createPCAModel(dt=l4C, fvNames = RUV3LoessMEPNames)
...
xlim<-c(-15,15)
ylim<-c(-10,10)
dt <- cbind(data.table(RUV3LoessMEPFBSPCAModel$x),Ligand=l4C$Ligand, MEP=l4C$MEP, ECMp=l4C$ECMp)
...
l4CU <- unique(l4C)
data_matrix <- scale(as.matrix(l4CU[,RUV3LoessMEPNames, with=FALSE]))
...
data_matrix <- scale(as.matrix(l4C[,RUV3LoessMEPNames, with=FALSE]))
...
plotLEHmap(l4C, hclustfun = "hclust", fill="Spot_PA_SpotCellCountLog2RUV3Loess", titleExpression=titleExpression, limits=limits)
