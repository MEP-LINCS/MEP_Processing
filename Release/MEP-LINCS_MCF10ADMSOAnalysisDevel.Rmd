...
fillRow(height = "100%", flex = c(NA, 1), 
        ...
        output$scatterPlot <- renderPlot({
          ...
          #xaxisDescription <- dtSignals$Description[dtSignals$Name==input$xaxis]
          #yaxisDescription <- dtSignals$Description[dtSignals$Name==input$yaxis]
          
          p <- ggplot(dt, aes_string(x=xaxisBinding, y= yaxisBinding))+
            geom_point()+
            labs(x=input$xaxis, y=input$yaxis)
          p
        })
        
        #output$xaxisDescription <- renderText({dtSignals$Description[dtSignals$Name==input$xaxis]}),
         #output$yaxisDescription <- renderText({dtSignals$Description[dtSignals$Name==input$yaxis]})
)
```
