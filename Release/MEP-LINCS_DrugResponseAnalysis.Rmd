---
title: MEP-LINCS MCF10A Neratinib Analysis 
output: 
...
textOrdered <- levels(reorder(dt$Ligand, dt$Nuclei_PA_Cycle_DNA2NProportionLogitRUVLoessBacktransformed, FUN=median))
...
p <- ggplot(dt, aes(x=reorder(Ligand, Nuclei_PA_Cycle_DNA2NProportionLogitRUVLoessBacktransformed, FUN=median), y=Nuclei_PA_Cycle_DNA2NProportionLogitRUVLoessBacktransformed, fill=gsColours))+
...
***
This data is RUV and loess normalized and then summarized. The NID1 and ELN data are filtered due to low cell counts that create extreme values.  
...
colNames <- c("MEP","Nuclei_PA_Gated_EdUPositiveProportionLogitRUVLoessBacktransformed", "Spot_PA_SpotCellCountLog2RUVLoess", "Nuclei_PA_Cycle_DNA2NProportionLogitRUVLoessBacktransformed","Cytoplasm_CP_Intensity_MedianIntensity_KRT5Log2RUVLoess")
colNamesDisplay <- c("MEP","Normed EdU+ Prop.","Normed SCC (log2)","Normed DNA 4N Prop.", "Normed KRT5")
...
***
The boxplots show the RUV and loess normalized MEP level data for the KRT5 cytoplasmic median intensity signal stratifed by the ligands.  
...
dt <- l4C[!grepl("NID1|ELN",l4C$ECMp),]
p <- ggplot(dt, aes(x=Spot_PA_SpotCellCount, y=btLogit(Nuclei_PA_Gated_EdUPositiveProportionLogitRUVLoess), colour=Ligand, text=paste("ECMp:",ECMp)))+
...
###KRT5 vs. Proliferation Scatterplot


```{r, fig.width=10, fig.height=7, eval=TRUE}

dt <- l4C[!grepl("NID1|ELN",l4C$ECMp),]
p <- ggplot(dt, aes(x=btLogit(Nuclei_PA_Gated_EdUPositiveProportionLogitRUVLoess), y=Cytoplasm_CP_Intensity_MedianIntensity_KRT5Log2RUVLoess, colour=Ligand, text=paste("ECMp:",ECMp)))+
  geom_point(size=.7, alpha=.4)+
  guides(colour=FALSE, size=FALSE)+
  #coord_cartesian(ylim = c(7,13))+
  xlab("Normalized EdU+")+ylab("Normalized KRT5+")+
...
***
This scatter plot shows the normalized KRT5+ proportion vs. the normalized EdU+ measured proliferation. The colors represent the ligands. The ECM protein is displayed in the tool tip box that appears while hovering over a spot.  


```{r PCAData, eval=FALSE}
...
###Spot Level KRT5 Responses

```{r, fig.width=20, fig.height=8}
dt <- l3C[grepl("BMP",Ligand),]

p <- ggplot(dt, aes(x=reorder(ECMp, Cytoplasm_CP_Intensity_MedianIntensity_KRT5Log2RUVLoess, FUN=median), y=Cytoplasm_CP_Intensity_MedianIntensity_KRT5Log2RUVLoess))+
  geom_boxplot(outlier.colour = NA, alpha=.5)+
  guides(fill=FALSE, colour=FALSE)+
  #coord_cartesian(ylim = c(-8,0))+
  xlab("ECMp")+ylab("Normalized KRT5")+
  ggtitle("Spot level KRT5 by ECMp")+
  facet_wrap(~Ligand, ncol=3)+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size=rel(.7)), axis.text.y = element_text(angle = 0, vjust = 0.5, hjust=1, size=rel(1)), plot.title = element_text(size = rel(1)),legend.text=element_text(size = rel(1)),legend.title=element_text(size = rel(1)))
...
These boxplots show the gated KRT5  intensity values for the replicate ECM protein spots paired with the BMP family of ligands.
...
p <- ggplot(dt, aes(x=reorder(ECMp, Nuclei_PA_Cycle_DNA2NProportionLogitRUVLoessBacktransformed, FUN=median), y=Nuclei_PA_Cycle_DNA2NProportionLogitRUVLoessBacktransformed))+
  geom_boxplot(outlier.colour = NA, alpha=.5)+
  guides(colour=FALSE)+
  #coord_cartesian(ylim = c(-8,0))+
...
limits=c(.05, .95)
titleExpression <-"Raw Spot Cell Count"
plotLEHmap(l4C,  hclustfun = "hclust",fill="Spot_PA_SpotCellCount", titleExpression=titleExpression, limits=limits)
```
...
```{r}
limits=c(.05, .95)
titleExpression <-"Normalized Spot Cell Count"
