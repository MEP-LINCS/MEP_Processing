...
Each well is scored for even cell seeding according to the count of the DAPI-stained nuclei. A detailed explanation of the QA method is in the supplemental material. In brief, the level 3 data have cell counts at the spot level and locally-averaged cell counts of the neighborhoods. Both of these parameters are used to score the wells and filter the dataset. QA Scores range from 0 to 1 and represent the proportion of the spots that have at least one cell and are not in low cell count neighborhoods. Wells with QA scores less than `r wellQAThresh` are flagged as low quality wells and the data is removed in particular analyses.

The following plots are pseudoimages of each MEMA's spot cell count and a histogram of the loess model with its QA score. 
...
dt <- l4F
dt <- l4F[,list(Barcode,Ligand,Spot_PA_SpotCellCount, Spot_PA_SpotCellCountLog2RUV, Spot_PA_SpotCellCountLog2RUVLoess )]
dt <- dt[,Spot_PA_SpotCellCountLog2:= log2(Spot_PA_SpotCellCount)]
dt <- melt(dt,id.vars=c("Barcode","Ligand"),measure.vars=c("Spot_PA_SpotCellCountLog2","Spot_PA_SpotCellCountLog2RUV","Spot_PA_SpotCellCountLog2RUVLoess"), variable.name = "Processed",value.name = "SpotCellCount", variable.factor = FALSE)
dt$Processed[!grepl("RUV",dt$Processed)] <- "RawLog2"
dt$Processed[grepl("RUV$",dt$Processed)] <- "RUV"
...
p <- ggplot(dt, aes(x=Nuclei_PA_Gated_EdUPositiveProportionLogitRUVLoess))+geom_histogram(binwidth = .02)+
...
```{r ProliferationByLigandandKRT19Full, eval=grepl("SS4",ss)&FALSE}

p <- ggplot(dt, aes(x = reorder(Ligand, Nuclei_PA_Gated_EdUPositiveProportionLogitRUVLoess, FUN=median), y = Nuclei_PA_Gated_EdUPositiveProportionLogitRUVLoess, colour = Cytoplasm_PA_Gated_KRT19Positive))+geom_boxplot()+
...
p <- ggplot(dt, aes(x = reorder(Ligand, Nuclei_PA_Gated_EdUPositiveProportionLogitRUVLoess, FUN=median), y = Nuclei_PA_Gated_EdUPositiveProportionLogitRUVLoess, colour = Barcode))+geom_boxplot()+
  ggtitle(paste("\n\nMEP Normalized, EdU+ Proportion by Ligand (full dataset)"))+
  xlab("")+ylab("")+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size=rel(.8)), axis.text.y = element_text(angle = 0, vjust = 0.5, hjust=1, size=rel(1)), plot.title = element_text(size = rel(1)),legend.text=element_text(size = rel(.3)),legend.title=element_text(size = rel(.3)))
suppressWarnings(print(p))
...
p <- ggplot(dt, aes(x = reorder(ECMp, Nuclei_PA_Gated_EdUPositiveProportionLogitRUVLoess, FUN=median), y = Nuclei_PA_Gated_EdUPositiveProportionLogitRUVLoess))+geom_boxplot()+
...
```{r ProliferationHistogram, eval=grepl("SS2|SS4|SS6",ss)}
...
```{r ProliferationByLigand, eval=grepl("SS2|SS4|SS6",ss)}
...
```{r ProliferationByECMp,eval=grepl("SS2|SS4|SS6",ss)}
