...
###Introduction
The MEP-LINCS `r cellLine` `r ss` experiment was performed with cells grown in `r length(barcodes)` 8-well plates. The `r ss` staining set includes, DAPI, `r unique(l4$Endpoint488)`, `r unique(l4$Endpoint555)` and `r unique(l4$Endpoint647)`. Color images of the cells at each spot were gathered on a Nikon automated microscope. The Cell Profiler Pipeline version `r rawDataVersion` performed the segmentation and the analysis pipeline version is `r analysisVersion`.
...
for (barcode in barcodes){
  ...
  if (grepl("SSE",ss)){
    ...
    p <- create8WellPseudoImage(DT, pr = "Nuclei_CP_Intensity_IntegratedIntensity_Nestin", prDisplay = unique(DT$Endpoint488))
    suppressWarnings(print(p))
