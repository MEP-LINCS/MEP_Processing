...
l4$Spot_PA_SpotCellCountLog2 <- boundedLog2(l4$Spot_PA_SpotCellCount)
...
###Introduction
The MEP-LINCS `r cellLine` `r ss` experiment was performed with cells grown in `r length(barcodes)` 8-well plates. The `r ss` staining set channels are `r  combine_words(gsub("Endpoint","",grep("Endpoint",colnames(l3),value=TRUE)))`. The endpoints are `r combine_words(unique(l3[,grep("Endpoint",colnames(l3),value=TRUE), with=FALSE]))`. Color images of the cells at each spot were gathered on a Nikon automated microscope. The Cell Profiler Pipeline version `r rawDataVersion` performed the segmentation and the analysis pipeline version is `r analysisVersion`.
...
p <- ggplot(l3, aes(x=factor(Ligand), y=Spot_PA_SpotCellCount, colour=factor(Barcode)))+
  ...
  #guides(colour=FALSE)+
  xlab("")+ylab("Spot Cell Count")+
  ggtitle("Spot Cell Count by Ligand and Barcode ")+
  theme(axis.text.x = element_text(angle = 90, vjust = 0, hjust=0.5, size=rel(1)), axis.text.y = element_text(angle = 0, vjust = 0.5, hjust=1, size=rel(1)), plot.title = element_text(size = rel(1)),legend.text=element_text(size = rel(.3)),legend.title=element_text(size = rel(.8)))
p <- p + geom_hline(yintercept = median(l3$Spot_PA_SpotCellCount), colour="blue", alpha=.5)
print(p)
...
```{r, child='MEP-LINCS_EdUText.Rmd', eval=grepl("SS2|SS4|SS6|SSA|SSE|SSF",ss)}
```

<br>

```{r rawEdUBoxplots, fig.height=5, eval=grepl("SS2|SS4|SS6|SSA|SSD|SSE|SSF",ss)}
...
```{r rawEdUBoxplotsbyWell, fig.height=5, eval=grepl("SS2|SS4|SS6|SSA|SSD|SSE|SSF",ss)}
...
```{r rawEdUBoxplotsbyLigand, fig.height=5, eval=grepl("SS2|SS4|SS6|SSA|SSD|SSE|SSF",ss)}

p <- ggplot(l3, aes(x=factor(Ligand), y=Nuclei_PA_Gated_EdUPositiveProportion, colour=factor(Barcode)))+
  geom_boxplot(outlier.size = .5)+
  xlab("")+ylab("EdU+ Proportion")+
  ggtitle("EdU+ Proportion per Spot by Ligand and Barcode ")+
  theme(axis.text.x = element_text(angle = 90, vjust = 0, hjust=0.5, size=rel(1)), axis.text.y = element_text(angle = 0, vjust = 0.5, hjust=1, size=rel(1)), plot.title = element_text(size = rel(1)),legend.text=element_text(size = rel(.3)),legend.title=element_text(size = rel(.8)))
...
```{r, child='MEP-LINCS_ProliferationText.Rmd', eval=grepl("SS2|SS4|SS6|SSA",ss)}
```
...
```{r, eval=grepl("SS6|SSA|SSD|SSE",ss)}
...
```{r, , fig.height=10, eval=grepl("SS2|SS4|SS6|SSA|SSD|SSE",ss)}
