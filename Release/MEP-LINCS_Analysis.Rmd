...
```{r, child='MEP-LINCS_RUV3ResidualsText.Rmd', eval=unique(l3$NormMethod)=="RUV3LoessResiduals" & grepl("SS3|SS4",ss)}
```
...
#Cast to get ligands into columns
df <- dcast(data.frame(l4HF[,list(ECMp,Ligand,Cytoplasm_CP_Intensity_MedianIntensity_MitoTrackerLog2RUV3Loess,Barcode)]),ECMp~Ligand, value.var = "Cytoplasm_CP_Intensity_MedianIntensity_MitoTrackerLog2RUV3Loess")
...
```{r NormedProliferationByMEPHF,eval=grepl("SS2|SS4|SS6",ss), fig.width=8, fig.height=6}
...
```{r EdUHeatmapHF, fig.width=8, fig.height=5, eval=grepl("SS2|SS4|SS6",ss)}
...
```{r LineagRatioHeatmapHF, fig.width=8, fig.height=5, eval=grepl("SS3|SS4",ss)}
