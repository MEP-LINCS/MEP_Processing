...
if (grepl("SS2|SS4|SSA|SSE|SSF",StainingSet)){
...
p <- ggplot(dt, aes(x =reorder(MEP, Spot_PA_SpotCellCountLog2RUVLoess), y = Spot_PA_SpotCellCountLog2RUVLoess))+
  ...
  xlab("MEP")+ylab("Normalized Spot Cell Count (log2)")+
...
The interactive heatmaps below are arranged by unsupervised clustering of the rows and columns and colored by the normalized log2 of the spot cell count. Clicking and dragging across any subsection will zoom in on that section. Double clicking on the zoomed image will return to the full heatmap.  
...
The first datatable below shows the MEP summarized values while the second table shows the spot-level data. The spot-level datatable includes links to the images stored in OHSU's Omero database (account needed).  
...
A useful mining method is to use the first table to identify a MEP of interest as this table uses robust methods to summarize the replicates, then view the individual replicate data and images in the second table.  

<br> 


```{r SCCTable}
if(any(grepl("SS1",StainingSet))){
...
} else if (any(grepl("SS2|SS4|SS6|SSA",StainingSet))){
...
} else if (any(grepl("SSD",StainingSet))){
...
} else if (any(grepl("SSE",StainingSet))){
...
} else if (any(grepl("SSF",StainingSet))){
...
} else if (any(grepl("SS3|SS4",StainingSet))){
  if (CellLine=="MCF10A"){
...
p <- ggplot(dt, aes(x =reorder(MEP, Nuclei_PA_Cycle_DNA2NProportionLogitRUVLoess), y = btLogit(Nuclei_PA_Cycle_DNA2NProportionLogitRUVLoess)))+
  geom_errorbar(aes(ymin=btLogit(Nuclei_PA_Cycle_DNA2NProportionLogitRUVLoess-Nuclei_PA_Cycle_DNA2NProportionLogitRUVLoess_SE), ymax=btLogit(Nuclei_PA_Cycle_DNA2NProportionLogitRUVLoess+Nuclei_PA_Cycle_DNA2NProportionLogitRUVLoess_SE)), width=.01, colour="black") +
  xlab("MEP")+ylab("Normalized DNA 2n Proportion Ratio")+
  theme( axis.text.x=element_blank(), axis.ticks=element_blank(),  panel.grid.major = element_blank())+
  ggtitle("MEPs Ordered by Normalized DNA Proportion")

p <- p + geom_point(aes(y=btLogit(Nuclei_PA_Cycle_DNA2NProportionLogitRUVLoess)),colour = "blue", alpha = .5)
...
The interactive heatmaps below are arranged by unsupervised clustering of the rows and columns and colored by the logits of the normalized DNA 2n proportions. Clicking and dragging across any subsection will zoom in on that section. Double clicking on the zoomed image will return to the full heatmap.    

```{r DNA2NHeatmap, fig.width=8, fig.height=5}
...
```{r MitoTrackerByMEPFull, eval=grepl("SS1",StainingSet), fig.width=8, fig.height=6}
dt <- l4F
...
```{r MitotrackerHeatmap, eval=grepl("SS1",StainingSet), fig.width=8, fig.height=5}
...
```{r, child='MEP-LINCS_ProliferationText.Rmd', eval=grepl("SS2|SS4|SSE|SSA",StainingSet)}
```



```{r NormedProliferationByMEP,eval=grepl("SS2|SS4|SS6|SSA|SSE|SSF",StainingSet), fig.width=8, fig.height=6}

dt <- l4F

p <- ggplot(dt, aes(x =reorder(MEP, Nuclei_PA_Gated_EdUPositiveProportionLogitRUVLoess), y = btLogit(Nuclei_PA_Gated_EdUPositiveProportionLogitRUVLoess)))+
  geom_errorbar(aes(ymin=btLogit(Nuclei_PA_Gated_EdUPositiveProportionLogitRUVLoess-Nuclei_PA_Gated_EdUPositiveProportionLogitRUVLoess_SE), ymax=btLogit(Nuclei_PA_Gated_EdUPositiveProportionLogitRUVLoess+Nuclei_PA_Gated_EdUPositiveProportionLogitRUVLoess_SE)), width=.01, colour="black") +
...
```{r, child='MEP-LINCS_ProliferationHeatMapsText.Rmd', eval=grepl("SS2|SS4|SSA|SSE|SSF",StainingSet)}
```


```{r EdUHeatmapFull, fig.width=8, fig.height=5, eval=grepl("SS2|SS4|SS6|SSA|SSE|SSF",StainingSet)}
...
try(d3heatmap(dfZoom(df, .02, .98), colors=hmcols, xaxis_font_size="6pt", yaxis_font_size="5pt"), TRUE)

```


```{r EdUBoxplotsFull, fig.width=12, fig.height=5, eval=grepl("SS2|SS4|SS6|SSA|SSE|SSF",StainingSet)}
...
```{r NormedNestinByMEP,eval=grepl("SSE",StainingSet), fig.width=8, fig.height=6}
...
```{r NestinHeatmapFull, fig.width=8, fig.height=5, eval=grepl("SSE",StainingSet)}
...
```{r NestinBoxplotsFull, fig.width=12, fig.height=5, eval=grepl("SSE",StainingSet)}
...
```{r LineageRatioByMEP,eval=grepl("SS3|SS4",StainingSet), fig.width=8, fig.height=6}
...
```{r LineageMarkers, echo=FALSE, fig.width=8, fig.height=6,eval=grepl("SS3|SS4",StainingSet)}
...
```{r LineagRatioHeatmapFull, fig.width=8, fig.height=5, eval=grepl("SS3|SS4",StainingSet)}
...
```{r EdUvsLineageMarkers, echo=FALSE, fig.width=8, fig.height=6,eval=grepl("SS4",StainingSet)}
...
fvDT <- filterl4RUV(l4F, lowQALigands)
setkey(fvDT, MEP)
fvDT <- fvDT[!grepl("air|Fiducial|fiducial|blank|PBS",fvDT$MEP),]
...
fvDT <- fvDT[,!(names(fvDT) %in% c("Barcode","MEP"))]
```
