...
```{r readLevel1}
l1 <- fread(paste0("../AnnotatedData/",datasetName,"_",ss,"_Level1.txt"), showProgress = FALSE)
```
...
##Introduction
The LINCS `r cellLine` `r ss` experiment was performed with cells grown in `r length(barcodes)` 8-well plates. The `r ss` staining set channels are `r  combine_words(gsub("Endpoint","",grep("Endpoint",colnames(l1),value=TRUE)))`. The endpoints are `r combine_words(unique(l1[,grep("Endpoint",colnames(l1),value=TRUE), with=FALSE]))`. Color images of the cells at each spot were gathered on a Nikon automated microscope. The CP Pipeline `r rawDataVersion` performed the segmentation and the analysis pipeline is `r analysisVersion`.
...
```{r rawCaspaseHistograms, fig.width=3.7,fig.height=4, eval=exists("l1")&grepl("SSD",ss)}
binwidth=.001
for (barcode in barcodes){
  setkey(l1S, Barcode)
  dtb <- l1S[barcode]
...
```{r rawEdUHistograms, fig.width=3.7,fig.height=4, eval=exists("l1")&grepl("SS2|SS4|SS6|SSA|SSD|SSE|SSF",ss)}
...
```{r rawKrt5Histograms, fig.width=3.7,fig.height=4, eval=exists("l1")&grepl("SS3|SS4|SSF",ss)}
...
```{r gatedKRT5Ligandboxplots, fig.width=10,fig.height=4, eval=exists("l1")&grepl("SS3|SS4",ss)}
...
```{r EdUgatedKRT5Scatterplots, fig.width=8,fig.height=4, eval=exists("l1")&grepl("SSF",ss)}
...
```{r EdUbygatedKRT5Ligandboxplots, fig.width=10,fig.height=4, eval=exists("l1")&grepl("SSF",ss)}
...
```{r Cell_cycle_plots,echo=FALSE, fig.width=8, fig.height=5, eval=exists("l1")&grepl("SS2|SS4|SS6|SSA|SSD|SSE|SSF",ss)}
binwidth=.04
for (barcode in barcodes){
  setkey(l1S, Barcode)
  dtb <- l1S[barcode]
...
```{r KRT5EdU_plots,echo=FALSE, fig.width=8, fig.height=5, eval=exists("l1")&grepl("SS4",ss)}
binwidth=.04
for (barcode in barcodes){
  setkey(l1S, Barcode)
  dtb <- l1S[barcode]
