...
Most of histograms and scatterplots that display cell level data are from a randomly sampled subset of the full dataset (sample proportion = `r sampleProportion`).
...
xMin <- quantile(l1S$Cytoplasm_CP_Intensity_MedianIntensity_KRT14Log2,probs=.01, na.rm=TRUE)
xMax <- quantile(l1S$Cytoplasm_CP_Intensity_MedianIntensity_KRT14Log2,probs=.99, na.rm=TRUE)
...
```{r gatedKRTScatterplots, fig.width=8,fig.height=8, eval=exists("l1")&grepl("SS3|SS4",ss)}
...
```{r EdUgatedKRTScatterplots, fig.width=8,fig.height=8, eval=exists("l1")&grepl("SS3|SS4",ss)}
...
```{r EdUbygatedKRTLigandboxplots, fig.width=10,fig.height=4, eval=exists("l1")&grepl("SS3|SS4",ss)}
...
```{r rawCOL1KRT5Densities, fig.width=10,fig.height=10, eval=exists("l1")&grepl("SS3|SS4",ss)}
#Display density plots for one COl1 spot of each ligand
...
```{r rawCOL1KRT19Histograms, fig.width=10,fig.height=10, eval=exists("l1")&grepl("SS3|SS4",ss)}
#Display histograms for one COl1 spot of each ligand
