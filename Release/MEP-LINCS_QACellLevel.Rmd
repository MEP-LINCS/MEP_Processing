...
plateType <- paste0(length(unique(l1S$Well)),"Well")
nrPlateRow <- length(unique(gsub("[[:digit:]]","",l1S$Well)))
...
#Determine how to set a common scale for all figures
...
```{r rawKrt5Histograms, fig.width=3.7,fig.height=4, eval=any(grepl("KRT5",colnames(l1S)))}
...
```{r EdUgatedKRT5Scatterplots, fig.width=8,fig.height=4, eval=any(grepl("KRT5",colnames(l1S)))&any(grepl("EdU",colnames(l1S)))}
...
```{r EdUbygatedKRT5Ligandboxplots, fig.width=10,fig.height=4, eval=any(grepl("KRT5",colnames(l1S)))&any(grepl("EdU",colnames(l1S)))}
