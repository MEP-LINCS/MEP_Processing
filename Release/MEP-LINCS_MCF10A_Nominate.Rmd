...
###Cell Cycle 
The cell cycle boxplots are based on gating the cells on their total DAPI signals into 2N or 4N classes. This data is RUV3 and loess normalized and then summarized across all  staining sets where each ligand has three replicates and there are 24 control wells.  
...
The boxplots show the RUV3 and loess normalized MEP level data for the gated EdU+ proliferation signal stratifed by the ligands. 
...
The scatter plot below shows the normalized KRT5 median intensity vs the normalized EdU+ measured proliferation. The spot size represents the relative number of cells. The colors represent the ligands.
