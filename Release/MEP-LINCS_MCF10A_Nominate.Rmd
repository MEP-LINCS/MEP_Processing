...
plotPCs(m = RUV3LoessMEPPCAModel,PCs = c("PC1","PC2","PC3"),colour = "l4nMean$Ligand", title = "PCA of All MEP Signals\n", legendTitle = "Barcode", xlim = c(-15,15), ylim=c(-10,10), highlightName = "BMP2")
...
```{r MEPPCADatatable}
dt <- data.frame(RUV3LoessMEPPCAModel$x)
dt$MEP <- rownames(dt)
dt<- data.table(dt)
...
p <- p+geom_text(data=dt[grepl("^1$|^45$",ECMpRank)|grepl("COL1",ECMp),],aes(label=ECMp),nudge_x = 0.2)

p <- p+geom_text(data=dt[grepl("^2$|^46$",ECMpRank),],aes(label=ECMp),nudge_x = -0.2)
print(p)
...
p <- p+geom_text(data=dt[(grepl("^1$|^2$",ECMpRank)|grepl("COL1|NID1",ECMp))&grepl("HGF",Ligand),],aes(label=ECMp),nudge_x = 0.15)

p <- p+geom_text(data=dt[(grepl("^45$",ECMpRank)|grepl("COL1",ECMp))&grepl("FGF2",Ligand),],aes(label=ECMp),nudge_x = 0.2)

p <- p+geom_text(data=dt[grepl("^46$",ECMpRank)&grepl("FGF2",Ligand),],aes(label=ECMp),nudge_x = -0.2)
print(p)
...
p <- p+geom_text(data=dt[(grepl("^45$",ECMpRank)&grepl("Wnt5a",Ligand)),],aes(label=ECMp),nudge_x = -0.15)

p <- p+geom_text(data=dt[((grepl("^46$",ECMpRank)|grepl("COL1",ECMp))&grepl("Wnt5a",Ligand)),],aes(label=ECMp),nudge_x = 0.15)
