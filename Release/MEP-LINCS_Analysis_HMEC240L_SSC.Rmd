...
l3n <- addOmeroIDs(shortenHA(fread(unique(paste0("../AnnotatedData/",x[["cellLine"]], "_SSC","_Level3.txt")), showProgress = FALSE)))

l4 <- addOmeroIDs(shortenHA(fread(unique(paste0("../AnnotatedData/",x[["cellLine"]], "_SSC","_Level4.txt")), showProgress = FALSE)))

l3SS1 <- addOmeroIDs(shortenHA(fread(unique(paste0("../AnnotatedData/",x[["cellLine"]], "_SS1","_Level3.txt")), showProgress = FALSE)))

l3SS4 <- addOmeroIDs(shortenHA(fread(unique(paste0("../AnnotatedData/",x[["cellLine"]], "_SS4","_Level3.txt")), showProgress = FALSE)))

l4SS1 <- addOmeroIDs(shortenHA(fread(unique(paste0("../AnnotatedData/",x[["cellLine"]], "_SS1","_Level4.txt")), showProgress = FALSE)))

l4SS4 <- addOmeroIDs(shortenHA(fread(unique(paste0("../AnnotatedData/",x[["cellLine"]], "_SS4","_Level4.txt")), showProgress = FALSE)))
...
l3SS14 <- l3SS14[,Nuclei_PA_Gated_EdUPositiveProportionLogitRUVLoessBacktransformed :=  2^Nuclei_PA_Gated_EdUPositiveProportionLogitRUVLoess/(1+2^Nuclei_PA_Gated_EdUPositiveProportionLogitRUVLoess)]
l3SS14 <- l3SS14[,Nuclei_PA_Cycle_DNA4NProportionLogitRUVLoessBacktransformed :=  2^Nuclei_PA_Cycle_DNA4NProportionLogitRUVLoess/(1+2^Nuclei_PA_Cycle_DNA4NProportionLogitRUVLoess)]

l4nMean <- meanSummarizel4(l4)
l4Merge <- merge(l4SS1[!grepl("FBS",l4SS1$MEP),grep("MEP|MitoTracker",colnames(l4SS1), value=TRUE), with=FALSE],
                  l4SS4[!grepl("FBS",l4SS4$MEP),grep("MEP|EdU|KRT",colnames(l4SS4), value=TRUE), with=FALSE],by = "MEP")

l4MergeFBS <- merge(l4SS1[,grep("MEP|MitoTracker",colnames(l4SS1), value=TRUE), with=FALSE],
...
l4Merge <- l4Merge[,Nuclei_PA_Gated_EdUPositiveProportionLogitRUVLoessBacktransformed :=  2^Nuclei_PA_Gated_EdUPositiveProportionLogitRUVLoess/(1+2^Nuclei_PA_Gated_EdUPositiveProportionLogitRUVLoess)]
l4Merge <- l4Merge[,Nuclei_PA_Cycle_DNA4NProportionLogitRUVLoessBacktransformed :=  2^Nuclei_PA_Cycle_DNA4NProportionLogitRUVLoess/(1+2^Nuclei_PA_Cycle_DNA4NProportionLogitRUVLoess)]

l4MergeFBS <- merge(l4MergeFBS, ligandMetadataDT, by="Ligand")
l4MergeFBS <- unique(l4MergeFBS,by = "MEP")
l4MergeFBS <- l4MergeFBS[,Nuclei_PA_Gated_EdUPositiveProportionLogitRUVLoessBacktransformed :=  2^Nuclei_PA_Gated_EdUPositiveProportionLogitRUVLoess/(1+2^Nuclei_PA_Gated_EdUPositiveProportionLogitRUVLoess)]
l4MergeFBS <- l4MergeFBS[,Nuclei_PA_Cycle_DNA4NProportionLogitRUVLoessBacktransformed :=  2^Nuclei_PA_Cycle_DNA4NProportionLogitRUVLoess/(1+2^Nuclei_PA_Cycle_DNA4NProportionLogitRUVLoess)]
```
...
dt <- l4MergeFBS[!grepl("NID1|ELN",l4MergeFBS$ECMp),]
...
dt <- l4MergeFBS[!grepl("NID1|ELN",l4MergeFBS$ECMp),]
p <- ggplot(dt, aes(x=btLogit(Nuclei_PA_Gated_EdUPositiveProportionLogitRUVLoess), y=btLogit(Cytoplasm_PA_Gated_KRT19PositiveProportionLogitRUVLoess), colour=Ligand, size=Spot_PA_SpotCellCountLog2, text=paste("ECMp:",ECMp)))+
  geom_point(alpha=.4)+
...
rawFVNames <- grep("Log2|Logit", grep("ECMp|Ligand|MEP|Barcode|Well|StainingSet|Loess|RUV",colnames(l4nMean),invert = TRUE, value = TRUE), value=TRUE)
rawPCAModel <- createPCAModel(dt=l4nMean, fvNames = rawFVNames)

#Get a PCA model from the RUV data
RUVNames <- grep("Loess",grep("Log2RUV$|LogitRUV$",colnames(l4nMean), value=TRUE),value=TRUE,invert=TRUE)
RUVPCAModel <- createPCAModel(dt=l4nMean, fvNames = RUVNames)

#Get a PCA model from the RUVLoess data
RUVLoessNames <- grep("LoessRUV",grep("Log2RUVLoess$|LogitRUVLoess$",colnames(l4nMean), value=TRUE), value=TRUE, invert=TRUE)
RUVLoessPCAModel <- createPCAModel(dt=l4nMean, fvNames = RUVLoessNames)

#Get a PCA model from the RUVLoess of all signals
RUVLoessMEPNames <- grep("LoessRUV",grep("Log2RUVLoess$|LogitRUVLoess$",colnames(l4MergeFBS), value=TRUE), value=TRUE, invert=TRUE)
RUVLoessMEPPCAModel <- createPCAModel(dt=l4Merge, fvNames = RUVLoessMEPNames)

RUVLoessMEPFBSPCAModel <- createPCAModel(dt=l4MergeFBS, fvNames = RUVLoessMEPNames)
...
plotPCs(m = RUVLoessMEPFBSPCAModel,PCs = c("PC1","PC2","PC3"),colour = "l4nMean$Ligand", title = "PCA of All MEP Signals\n", legendTitle = "Barcode", xlim = c(-15,15), ylim=c(-10,10), highlightName = "BMP2")
...
####PCA Coloured by Barcode
```{r, fig.height=2, fig.width=3, eval =FALSE}
#Start debug here
p <- ggplot(data.frame(RUVLoessMEPFBSPCAModel$x), aes(x = PC1, y = PC2))+geom_point(size = rel(.5), alpha = .5)+
      guides(colour=FALSE)+
      theme(legend.text=element_text(size = 6),
            plot.title=element_text(size = rel(.7)))

p <- p + geom_point(l4MergeFBS, aes(colour=factor(Barcode)))
print(p)

plotPCs(m = RUVLoessMEPFBSPCAModel,PCs = c("PC1","PC2","PC3"),colour = "l4MergeFBS$Barcode", title = "PCA of All MEP Signals\n", legendTitle = "Barcode", xlim = c(-15,15), ylim=c(-10,10))
print(p)
...
```{r MEPPCADatatable}
dt <- data.frame(RUVLoessMEPFBSPCAModel$x)
dt$MEP <- rownames(dt)
dt<- data.table(dt)
...
data_matrix <- scale(as.matrix(l4MergeFBSU[,RUVLoessMEPNames, with=FALSE]))
...
data_matrix <- scale(as.matrix(l4Merge[,RUVLoessMEPNames, with=FALSE]))
...
p <- ggplot(l4Merge, aes(x=V1, y=V2, colour=Ligand, text=paste("ECMp:",ECMp)))+
  geom_point(size=rel(.2), alpha=.5)+
  ggtitle(paste("tSNE plot of HMEC240L MEPs Coloured by Ligand without FBS"))+
  guides(colour = FALSE)
ggplotly(p)
...
```{r plottSNEBarcodes,fig.width=8, fig.height=5, eval=FALSE}

p <- ggplot(l4Merge, aes(x=V1, y=V2, colour=Barcode, text=paste("Ligand:",Ligand,"ECMp:",ECMp)))+
  geom_point(size=rel(.2), alpha=.5)+
  ggtitle(paste("tSNE plot of HMEC240L MEPs Coloured by Barcode"))+
  guides(colour =FALSE)
ggplotly(p)
...
p <- ggplot(l4Merge, aes(x=V1, y=V2, colour=Ligand, text=paste("Ligand:",Ligand,"ECMp:",ECMp)))+
  geom_point(size=rel(.2), alpha=.5)+
  ggtitle(paste("tSNE plot Coloured by Ligand, seed:",seed))+
  guides(colour =FALSE)+
...
tmp <- lapply(RUVLoessMEPNames,function(sigName){
...
dt <- dt[,ECMpRank := rank(Nuclei_PA_Gated_EdUPositiveProportionLogitRUVLoessBacktransformed), by=Ligand]

p <- ggplot(dt, aes(x=reorder(Ligand, Nuclei_PA_Gated_EdUPositiveProportionLogitRUVLoessBacktransformed, FUN=median), y=Nuclei_PA_Gated_EdUPositiveProportionLogitRUVLoessBacktransformed))+
  geom_boxplot(outlier.colour = NA, alpha=.5)+
  guides(fill=FALSE, colour=FALSE)+
  #coord_cartesian(ylim = c(-6,-1.5))+
  xlab("Ligand")+ylab("Normalized EdU+")+
...
dt <- dt[,ECMpRank := rank(Cytoplasm_CP_Intensity_MedianIntensity_KRT5Log2RUVLoess), by=Ligand]

p <- ggplot(dt, aes(x=reorder(Ligand, Cytoplasm_CP_Intensity_MedianIntensity_KRT5Log2RUVLoess, FUN=median), y=Cytoplasm_CP_Intensity_MedianIntensity_KRT5Log2RUVLoess))+
  geom_boxplot(outlier.colour = NA, alpha=.5)+
  guides(fill=FALSE, colour=FALSE)+
  #coord_cartesian(ylim = c(-6,-1.5))+
  xlab("Ligand")+ylab("Normalized KRT5\n(Log2)")+
...
dt <- dt[,ECMpRank := rank(Nuclei_PA_Cycle_DNA4NProportionLogitRUVLoessBacktransformed), by=Ligand]

p <- ggplot(dt, aes(x=reorder(Ligand, Nuclei_PA_Cycle_DNA4NProportionLogitRUVLoessBacktransformed, FUN=median), y=Nuclei_PA_Cycle_DNA4NProportionLogitRUVLoessBacktransformed))+
  geom_boxplot(outlier.colour = NA, alpha=.5)+
  guides(fill=FALSE, colour=FALSE)+
  #coord_cartesian(ylim = c(-6,-1.5))+
  xlab("Ligand")+ylab("Normalized DNA4N")+
...
nomMEPSCurated <- (nomMEPS[,grep("Barcode|^Well$|^Spot$|Array|MEP|ECMp|^Ligand$|Cells_CP_AreaShape_AreaLog2|Nuclei_CP_AreaShape_AreaLog2|Cytoplasm_CP_Intensity_IntegratedIntensity_KRT5Log2|Cytoplasm_CP_Intensity_IntegratedIntensity_KRT19Log2|Cytoplasm_CP_Intensity_MedianIntensity_KRT5Log2|Cytoplasm_CP_Intensity_MedianIntensity_KRT19Log2|Nuclei_CP_Intensity_IntegratedIntensity_DapiLog2|Nuclei_CP_Intensity_IntegratedIntensity_EdULog2|Nuclei_CP_Intensity_MedianIntensity_DapiLog2|Nuclei_CP_Intensity_MedianIntensity_EdULog2|Nuclei_PA_Cycle_DNA2NProportionLogit|Nuclei_PA_Cycle_DNA4NProportionLogit|Nuclei_PA_Gated_EdUPositiveProportionLogit",colnames(nomMEPS), value=TRUE), with=FALSE])
    nomMEPSCurated <- nomMEPSCurated[,grep("RUV$|_SE|RLE$|ECMpRank",colnames(nomMEPSCurated), value=TRUE, invert=TRUE), with=FALSE]
...
p <- ggplot(dt, aes(x=reorder(ECMp, Nuclei_PA_Gated_EdUPositiveProportionLogitRUVLoessBacktransformed, FUN=median), y=Nuclei_PA_Gated_EdUPositiveProportionLogitRUVLoessBacktransformed, size=Spot_PA_SpotCellCountLog2RUVLoess))+
  geom_point(alpha=.5)+
  guides(fill=FALSE, colour=FALSE)+
...
p <- ggplot(dt, aes(x=Spot_PA_SpotCellCountLog2RUVLoess, y=Nuclei_PA_Gated_EdUPositiveProportionLogitRUVLoessBacktransformed, colour=ECMp))+
  geom_point(alpha=.5)+
  guides(fill=FALSE, colour=FALSE)+
  coord_cartesian(xlim = c(6.5,8))+
...
p <- ggplot(dt, aes(x=Nuclei_PA_Cycle_DNA4NProportionLogitRUVLoessBacktransformed, y=Nuclei_PA_Gated_EdUPositiveProportionLogitRUVLoessBacktransformed, colour=ECMp))+
  geom_point(alpha=.5)+
  guides(fill=FALSE, colour=FALSE)+
  #coord_cartesian(ylim = c(-6,-1.5))+
...
p <- ggplot(dt, aes(x=Spot_PA_SpotCellCountLog2RUVLoess, y=Nuclei_PA_Cycle_DNA4NProportionLogitRUVLoessBacktransformed, colour=ECMp))+
  geom_point(alpha=.5)+
  guides(fill=FALSE, colour=FALSE)+
  #coord_cartesian(xlim = c(6.5,8))+
...
p <- ggplot(dt, aes(x=Spot_PA_SpotCellCountLog2RUVLoess, y=Cytoplasm_CP_Intensity_MedianIntensity_KRT5Log2RUVLoess, colour=ECMp))+
  geom_point(alpha=.5)+
  guides(fill=FALSE, colour=FALSE)+
  coord_cartesian(xlim = c(6.5,8))+
...
limits=quantile(l4nMean[["Spot_PA_SpotCellCountLog2RUVLoess"]], probs=c(.005, .995), na.rm=TRUE)
titleExpression <-"Spot_PA_SpotCellCountLog2RUVLoess"
plotLEHmap(l4nMean, fill="Spot_PA_SpotCellCountLog2RUVLoess", titleExpression=titleExpression, limits=limits)
...
limits=c(.05, .95)
titleExpression <-"Spot_PA_SpotCellCountLog2RUVLoess"
plotLEHmap(l4nMean, hclustfun = "hclust", fill="Spot_PA_SpotCellCountLog2RUVLoess", titleExpression=titleExpression, limits=limits)
