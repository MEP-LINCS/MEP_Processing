...
if (ss == "SS2"){
  ...
  if(EduPPmad == 0) { l4F$EduPP_MedNorm_RobustZ <- 0
    ...
    l4F$EduPositiveProportion_MedNorm_RobustZ <- (l4F$EduPositiveProportion_MedNorm-EduPPMedian)/EduPPmad
  }
}
...
p <- ggplot(l4F, aes(x=Cytoplasm_PA_Intensity_LineageRatio))+
  geom_bar(binwidth = .1)+
  ggtitle("Histogram of Lineage Ratios")
print(p)

p <- ggplot(l4F, aes(x=Cytoplasm_PA_Intensity_LineageRatio_RobustZ))+geom_bar(binwidth = .1)+
  geom_vline(xintercept = c(-2,2), colour = "blue")+
...
p <- ggplot(l4F, aes(x = reorder(Ligand, Cytoplasm_PA_Intensity_LineageRatio, FUN=median), y = Cytoplasm_PA_Intensity_LineageRatio, colour = Barcode))+geom_boxplot()+
  ggtitle(paste("\n\nMEP LineageRatio by Ligand"))+
  xlab("")+ylab("")+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size=rel(.8)), axis.text.y = element_text(angle = 0, vjust = 0.5, hjust=1, size=rel(1)), plot.title = element_text(size = rel(1)),legend.text=element_text(size = rel(.3)),legend.title=element_text(size = rel(.3)))
suppressWarnings(print(p))

p <- ggplot(l4F, aes(x = reorder(ECMpAnnotID, Cytoplasm_PA_Intensity_LineageRatio, FUN=median), y = Cytoplasm_PA_Intensity_LineageRatio))+geom_boxplot()+
...
p <- ggplot(l4[l4$Ligand == "FBS"], aes(x = reorder(Ligand, Cytoplasm_PA_Intensity_LineageRatio, FUN=median), y = Cytoplasm_PA_Intensity_LineageRatio, colour = Barcode))+geom_boxplot()+
...
#Plot lineage ratios of control wells in each plate
p <- ggplot(l4[l4$Ligand == "FBS"], aes(x = reorder(Ligand, Cytoplasm_CP_Intensity_MedianIntensity_KRT19, FUN=median), y = Cytoplasm_CP_Intensity_MedianIntensity_KRT19, colour = Barcode))+geom_boxplot()+
...
#Plot lineage ratios of control wells in each plate
p <- ggplot(l4[l4$Ligand == "FBS"], aes(x = reorder(Ligand, Cytoplasm_CP_Intensity_MedianIntensity_KRT5, FUN=median), y = Cytoplasm_CP_Intensity_MedianIntensity_KRT5, colour = Barcode))+geom_boxplot()+
...
p <- ggplot(l4F, aes(x=Cytoplasm_PA_Intensity_LineageRatio_MedNorm_RobustZ))+geom_bar(binwidth = .1)+
  geom_vline(xintercept = c(-2,2), colour = "blue")+
...
p <- ggplot(l4F, aes(x = reorder(Ligand, Cytoplasm_PA_Intensity_LineageRatio_MedNorm, FUN=median), y = Cytoplasm_PA_Intensity_LineageRatio_MedNorm, colour = Barcode))+geom_boxplot()+
  ggtitle(paste("\n\nMEP Normalized Lineage Ratio by Ligand"))+
  xlab("")+ylab("")+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size=rel(.8)), axis.text.y = element_text(angle = 0, vjust = 0.5, hjust=1, size=rel(1)), plot.title = element_text(size = rel(1)),legend.text=element_text(size = rel(.3)),legend.title=element_text(size = rel(.3)))
suppressWarnings(print(p))

p <- ggplot(l4F, aes(x = reorder(Ligand, Cytoplasm_PA_Intensity_LineageRatio_MedNorm, FUN=median), y = Cytoplasm_PA_Intensity_LineageRatio_MedNorm))+geom_boxplot()+
  ggtitle(paste("\n\nMEP Normalized Lineage Ratio by Ligand"))+
  ylim(.6,1.2)+
  xlab("")+ylab("")+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size=rel(.8)), axis.text.y = element_text(angle = 0, vjust = 0.5, hjust=1, size=rel(1)), plot.title = element_text(size = rel(1)),legend.text=element_text(size = rel(.3)),legend.title=element_text(size = rel(.3)))
...
p <- ggplot(l4F, aes(x = reorder(ECMpAnnotID, Cytoplasm_PA_Intensity_LineageRatio_MedNorm, FUN=median), y = Cytoplasm_PA_Intensity_LineageRatio_MedNorm))+geom_boxplot()+
...
p <- ggplot(l4F, aes(x=Nuclei_PA_AreaShape_Neighbors_MedNorm,
                     y = Cytoplasm_PA_Intensity_LineageRatio_MedNorm))+
  geom_point(alpha=.5)+
...
#Get the PCFVs
l4PCAModel <- prcomp(as.formula(paste(" ~ ", paste(grep("MedNorm",colnames(l4QA),value=TRUE), collapse = "+"))), data = l4QA[!grepl("NID1",l4QA$MEP)], scale= TRUE)

#Extract the first 9 PCs of each MEP into a matrix
l4pcvDT <- data.table(l4PCAModel$x[,1:nrPCs],Barcode =l4QA$Barcode[!grepl("NID1",l4QA$MEP)], MEP = l4QA$MEP[!grepl("NID1",l4QA$MEP)])

#Plot the heat map of the entire dataset
activeMEPs <- ubHeatmap(l4pcvDT, title = "MEP PCA Vectors\n(Without NID1)", cols = plateCol, activeThresh = 0 )
