...
# Find GAL file
q <- sprintf("select id from file where parentId=='syn4624341' and fileType=='gal' and CellLine=='%s' and StainingSet=='%s'", cl, ss)
qr <- synQuery(q)
galId <- qr$file.id

# Find XML file
q <- sprintf("select id from file where parentId=='syn4624341' and fileType=='xml' and CellLine=='%s' and StainingSet=='%s'", cl, ss)
qr <- synQuery(q)
xmlId <- qr$file.id
```
...
q <- sprintf("select id,name,Well,Barcode,dataSubType from file where parentId=='syn4624343' and level==0 and CellLine=='%s' and StainingSet=='%s'", cl, ss)

qdata <- synQuery(q)

barcodes <- qdata$file.Barcode
wells <- qdata$file.Well

processByWell <- function(well, plateDataFiles) {
  #browser()
  # print(well)
  wellDataFiles <- filter(plateDataFiles, file.Well == well)
  mainDataFile <- filter(wellDataFiles, file.dataSubType == "Main") 
  cytoDataFile <- filter(wellDataFiles, file.dataSubType == "Cyto")
  
  #Read in and merge the main and cyto data for each well
  mainDT <- convertColumnNames(fread(getFileLocation(synGet(mainDataFile$file.id)),
  ...
  if (nrow(cytoDataFile) > 0) {
  cytoDT <- convertColumnNames(fread(getFileLocation(synGet(cytoDataFile)),
                                     stringsAsFactors=FALSE))
...
processByBarcode <- function(barcode) {
  plateDataFiles <- filter(qdata, file.Barcode == barcode)
  wells <- unique(plateDataFiles$file.Well)
  # print(barcode)
  wellDataList <- lapply(wells, processByWell, plateDataFiles=plateDataFiles)
  
  #Create the cell data.table with spot metadata for the plate 
  pcDT <- rbindlist(wellDataList, fill = TRUE)
  
  #Read the well metadata
  wellQ <- sprintf("SELECT DISTINCT CellLine,Barcode,Well,LigandAnnotID,Endpoint488,Endpoint555,Endpoint647,EndpointDAPI FROM syn4876459 WHERE Barcode='%s'", barcode)
  wellMetadata <- synTableQuery(wellQ)@values
...
expDTList <- lapply(barcodes[1:2], processByBarcode)
...
write.table(format(cDTLevel1, digits=4), 
            paste0(cl, "_", ss,"_","Level1.txt"), 
            sep = "\t",row.names = FALSE, quote=FALSE)

cDTLevel1SynFile <- File(paste0(cl, "_", ss,"_","Level1.txt"), parentId='syn4624344')

cDTLevel1SynFile <- synStore(cDTLevel1SynFile, used=qf$file.id, executed=thisScript)

write.table(format(cDTLevel2, digits=4), 
            paste0(cl,"_",ss,"_","Level2.txt"),
            sep = "\t", row.names = FALSE, quote=FALSE)

cDTLevel2SynFile <- File(paste0(cl, "_", ss,"_","Level2.txt"), parentId='syn4624344')

cDTLevel2SynFile <- synStore(cDTLevel2SynFile, used=cDTLevel1SynFile, executed=thisScript)
