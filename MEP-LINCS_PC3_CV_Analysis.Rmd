...
DT <- integrateSSCommon(SSs = c("SS1","SS2","SS3"), level = 3, cellLine = "PC3")
DT4 <- integrateSSCommon(SSs = c("SS1","SS2","SS3"), level = 4, cellLine = "PC3")
...
CVParameters <- grep("Center|MedNorm|_PA_",grep("Nuclei",numericParameters, value = TRUE),value = TRUE, invert = TRUE)
CVParameters <- c(CVParameters,"Spot_PA_SpotCellCount")

#Keep these metadata parameters
metadataNames <- grep("Barcode|ECMpAnnotID|Array|Block|PrintOrder",colnames(DT), value = TRUE)
DT <- cbind(DT[,metadataNames, with = FALSE],DT[,CVParameters, with = FALSE])

CVDT <- DT[,lapply(.SD, CV), by="Barcode,ECMpAnnotID"]
CVDT$Order <- "Actual"
...
ranCVDTList <- mclapply(1:permNumber, function(x, dt){
  ...
  ranCVDT <- dt[,lapply(.SD, CV), by="Barcode,ECMpAnnotID"]
...
Density plots of the CVs show the experimental data in black and the null hypotheses distributions in blue. Features with actual CV distributions (eccentricity, extent, median DAPI, perimeter) that are to the left of the null hypothesis show evidence of good signals. Features with CV distribitions that overlap the null hypothesis (area and integrated intensity DAPI) are poor quality signal. 

The CVs for Spot Cell Count and many of the signals dependent on integrated DAPI intensity (total DAPI) are to the right of the null hypothesis. This is suspected to be caused by the regions of high and low cell number regions and perhaps uneven staining in the arrays. The replicates are in a randomly distributed pattern that minimizes the number of repeat replicates in each 5x5 pin block. This means that each set of replicates will have spots in high and low region cell counts leading to higher CVs than the randomly chosen, null hypothesis 'replicates'.  
...
for(feature in featureVector){
  ...
  p <-  ggplot(CVDT[CVDT$Order == "Actual"], aes_string(x = feature))+
    ...
    geom_density(data = CVDT[CVDT$Order == "Random"], aes(x = Nuclei_CP_AreaShape_Area), fill = "blue", binwidth = .02, alpha = .5)+
    xlim(0,1)+
...
##Plate Heat Maps
The following are plate heat maps of selected features. 
...
for(feature in curatedFeatureVector){
  ...
  p <-  ggplot(DT, aes_string(x = "ArrayColumn", y = "ArrayRow", color = paste("log2(",feature,")")))+
    geom_point(size =3)+
    xlim(1,20)+ylim(35,1)+
...
The following boxplots show the curated feature vectors stratified by plate. Observations include high variance in the area and spot cell count signals and lower variance in the eccentricity and median intensity signal.
...
for(feature in curatedFeatureVector){
  ...
  p <-  ggplot(DT, aes_string(x="Barcode", y = paste("log2(",feature,")")))+
    geom_boxplot()+
    ggtitle(titleName)+
...
The following boxplots show the indivdual ECM protein responses of selected feature vectors stratified by plate.

```{r, fig.height=20}
p <-  ggplot(DT, aes(x=reorder(ECMpAnnotID, Nuclei_CP_AreaShape_Eccentricity, FUN=median), y = Nuclei_CP_AreaShape_Eccentricity))+
  geom_boxplot()+
  facet_wrap(~Barcode, ncol=4)+
...
```{r}
DT <- DT[, EccentricityVariance := var(Nuclei_CP_AreaShape_Eccentricity), by="Barcode,ECMpAnnotID"]
...
The following boxplots show the individual ECM protein responses of selected feature vectors stratified by ECM protein in a selected well.
...
```{r}
p <-  ggplot(DT[DT$Barcode == "LI8X00402"], aes(x=reorder(ECMpAnnotID, Nuclei_CP_AreaShape_Eccentricity, FUN=median), y = Nuclei_CP_AreaShape_Eccentricity))+
  geom_boxplot()+
  #xlim(1,20)+ylim(35,1)
  #scale_color_gradient(low="white",high="red")+
  facet_wrap(~Barcode, ncol=4)+
...
p <-  ggplot(DT[DT$Barcode == "LI8X00402"], aes(x=reorder(ECMpAnnotID, Nuclei_CP_AreaShape_Extent, FUN=median), y = Nuclei_CP_AreaShape_Extent))+
  geom_boxplot()+
  #xlim(1,20)+ylim(35,1)
  #scale_color_gradient(low="white",high="red")+
  facet_wrap(~Barcode, ncol=4)+
...
p <-  ggplot(DT, aes(x=reorder(ECMpAnnotID,Nuclei_CP_AreaShape_Area,FUN = median), y = Nuclei_CP_AreaShape_Area))+
  geom_boxplot()+
  #xlim(1,20)+ylim(35,1)+
  #scale_color_gradient(low="white",high="red")+
  facet_wrap(~Barcode, ncol=4)+
...
features =grep("Eccentricity|Extent|MedianIntensity_Dapi|Area$",colnames(CVDT), value = TRUE))
...
```{r}
setkey(DT4,Nuclei_CP_AreaShape_Eccentricity)
kable(DT4[DT4$Barcode=="LI8X00402", list(ECMpAnnotID,Spot_PA_SpotCellCount,Nuclei_CP_AreaShape_Eccentricity)], digits = 2)
```

```{r}

setkey(DT,Nuclei_CP_AreaShape_Eccentricity)
kable(DT[DT$Barcode=="LI8X00402", list(ECMpAnnotID,ArrayRow,ArrayColumn,Spot_PA_SpotCellCount,Nuclei_CP_AreaShape_Eccentricity)], digits = 2)
```
