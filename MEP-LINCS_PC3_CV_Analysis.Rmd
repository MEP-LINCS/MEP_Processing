...
permNumber <- 1000 #Used to create null hypotheses distributions
...
CVParameters <- grep("Center|MedNorm|_PA_|Euler|Orientation|Solidity",grep("Nuclei",numericParameters, value = TRUE),value = TRUE, invert = TRUE)
...
for(feature in curatedFeatureVector){
  ...
  p <-  ggplot(DT, aes_string(x="Barcode", y = paste("log2(",feature,")"), colour = "SS"))+
    geom_boxplot()+
    ggtitle(titleName)+
...
```{r, fig.height=20}
p <-  ggplot(DT, aes(x=reorder(ECMpAnnotID, Nuclei_CP_AreaShape_Eccentricity, FUN=median), y = Nuclei_CP_AreaShape_Eccentricity, colour = SS))+
  geom_boxplot()+
  facet_wrap(~Barcode, ncol=4)+
...
p <-  ggplot(DT, aes(x=reorder(ECMpAnnotID,Nuclei_CP_AreaShape_Area,FUN = median), y = Nuclei_CP_AreaShape_Area, colour = SS))+
  geom_boxplot()+
  #xlim(1,20)+ylim(35,1)+
  #scale_color_gradient(low="white",high="red")+
  facet_wrap(~Barcode, ncol=4)+
...
```{r}
DT <- DT[, AreaVariance := var(Nuclei_CP_AreaShape_Area), by="Barcode,ECMpAnnotID"]
...
```{r}
DT <- DT[, MedianDAPIVariance := var(Nuclei_CP_Intensity_MedianIntensity_Dapi), by="Barcode,ECMpAnnotID"]
...
```{r}
p <-  ggplot(DT[DT$Barcode == "LI8X00402"], aes(x=reorder(ECMpAnnotID, Nuclei_CP_AreaShape_Eccentricity, FUN=median), y = Nuclei_CP_AreaShape_Eccentricity, colour = SS))+
  geom_boxplot()+
  #xlim(1,20)+ylim(35,1)
  #scale_color_gradient(low="white",high="red")+
  facet_wrap(~Barcode, ncol=4)+
...
p <-  ggplot(DT[DT$Barcode == "LI8X00402"], aes(x=reorder(ECMpAnnotID, Nuclei_CP_AreaShape_Extent, FUN=median), y = Nuclei_CP_AreaShape_Extent, colour = SS))+
  geom_boxplot()+
  #xlim(1,20)+ylim(35,1)
  #scale_color_gradient(low="white",high="red")+
  facet_wrap(~Barcode, ncol=4)+
