...
getl4HF <- function(cellLine, analysisVersion){
  ...
  l3 <- fread(paste0(cellLine, "_",ss,"_",analysisVersion,"_Level3.txt"), showProgress = FALSE)
...
for(cellLine in c("PC3","MCF7")){
  ...
  DT <- l4HFDT[cellLine]
  setkey(DT,Selected)
  p <- ggplot(DT,aes(x =reorder(MEP, Nuclei_PA_Gated_EduPositiveLogit_Norm), ymin=Nuclei_PA_Gated_EduPositiveLogit_Norm-Nuclei_PA_Gated_EduPositiveLogit_Norm_SE, ymax=Nuclei_PA_Gated_EduPositiveLogit_Norm+Nuclei_PA_Gated_EduPositiveLogit_Norm_SE))+
    ...
    ggtitle(paste("\n\nValidation MEPs in",cellLine,"Ordered by EdU+ Proportion"))
  p <- p+geom_point(aes(y = Nuclei_PA_Gated_EduPositiveLogit_Norm, colour=SelectedFrom, size=Selected,  shape=Selected), alpha = 1)
  print(p)
}
