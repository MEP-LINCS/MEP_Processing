...
fv <- grep(fvSS1, colnames(l4QA), value = TRUE)
...
p <- ggplot(data.frame(l4PCAModel$x), aes(x = PC1, y = PC2, col = l4QA$Cytoplasm_CP_Intensity_IntegratedIntensity_MitoTracker_RZSNorm)) +
  geom_point(size = rel(.8), alpha = .8) +
...
for (barcode in barcodes){
  ...
  p <- create8WellPseudoImage(DT, pr = "Cytoplasm_CP_Intensity_MedianIntensity_MitoTracker", prDisplay = unique(DT$Endpoint488))
  suppressWarnings(print(p))
  
    p <- create8WellPseudoImage(DT, pr = "Cytoplasm_CP_Intensity_MedianIntensity_Actin",prDisplay = unique(DT$Endpoint555))
  suppressWarnings(print(p))
