...
p <- ggplot(l4F, aes(x = reorder(ECMpAnnotID, SpotCellCount_MedNorm_RobustZ, FUN=median), y = SpotCellCount_MedNorm_RobustZ))+geom_boxplot()+
...
l4fv <- l4[!"blank"]
l4fv <- l4fv[!"fiducial"]
l4fv$MEP <- paste(l4fv$ECMpAnnotID, l4fv$LigandAnnotID, sep="-")

#Get numeric features
fv <- grep("_CP_|SpotCellCount|MEP$",colnames(l4fv),value=TRUE)
...
splits <- limma::strsplit2(activeFV$MEP,split="_")
rownames(activeFVM) <- paste(splits[,1], limma::strsplit2(splits[,2],"-")[,2], sep="_")
         
#TODO: get the barcode for each ligand and add color bar
ligands <- limma::strsplit2(rownames(activeFVM),"_")[,2]
aFVMBarcodes <- unique(l4$Barcode[ligands[33] == l4$Ligand])

#Cluster the active MEPs, scaling the inputs
plot.new()
heatmap.2(activeFVM,scale="column", col = bluered, trace = "none", cexRow=.5, cexCol=.7, key=FALSE, main = paste("Active MEP Feature Vectors"), mar = c(17, 10))
...
splits <- limma::strsplit2(activeFVF$MEP,split="_")
rownames(activeFVMF) <- paste(splits[,1], limma::strsplit2(splits[,2],"-")[,2], sep="_")
         
#Cluster the active MEPs, scaling the inputs
plot.new()
heatmap.2(activeFVMF,scale="column", col = bluered, trace = "none", cexRow=.5, cexCol=.7, key=FALSE, main = paste("Active MEP Feature Vectors (no NID1)"), mar = c(17, 10))

```


The next heatmap shows the same analysis run on the dataset after removing NID1 and using data normlaized to each plate's control well.
...
#Get the normlaized numeric features
...
splits <- limma::strsplit2(activeFVFN$MEP,split="_")
rownames(activeFVMFN) <- paste(splits[,1], limma::strsplit2(splits[,2],"-")[,2], sep="_")
         
#Cluster the active MEPs, scaling the inputs
plot.new()
heatmap.2(activeFVMFN,scale="column", col = bluered, trace = "none", cexRow=.5, cexCol=.7, key=FALSE, main = paste("Active Normalized MEP Feature Vectors (no NID1)"), mar = c(17, 10))
