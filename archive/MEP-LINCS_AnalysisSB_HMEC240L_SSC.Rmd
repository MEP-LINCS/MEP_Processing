---
title: MEP-LINCS HMEC240L MEP Analysis 
output: 
...
l3n <- addOmeroIDs(shortenHA(fread(unique(paste0("../AnnotatedData/",x[["cellLine"]], "_SSR","_Level3.txt")), showProgress = FALSE)))

l4 <- addOmeroIDs(shortenHA(fread(unique(paste0("../AnnotatedData/",x[["cellLine"]], "_SSR","_Level4.txt")), showProgress = FALSE)))
...
fwrite(l4MergeFBS,file.path = unique(paste0("../AnnotatedData/",x[["cellLine"]], "_SSC","_Level4.txt")),sep = "\t")
...
RUV3LoessMEPNames <- grep("LoessRUV3",grep("Log2RUV3Loess$|LogitRUV3Loess$",colnames(l4MergeFBS), value=TRUE), value=TRUE, invert=TRUE)
...
xlim<-c(-15,15)
ylim<-c(-10,10)
dt <- cbind(data.table(RUV3LoessMEPFBSPCAModel$x),Ligand=l4MergeFBS$Ligand, MEP=l4MergeFBS$MEP, ECMp=l4MergeFBS$ECMp)
