---
title: MEP-LINCS MCF10A MEP-LINCS Analysis
author: Mark Dane
output: 
...
l3C <- fread(unique(paste0("../AnnotatedData/",datasetName, "_",ss,"_Level3.txt")), showProgress = FALSE)
...
l3C <- addOmeroIDs(l3C)
#Read in the MEP-Level dataset for the cell line
l4C <- fread(unique(paste0("../AnnotatedData/",datasetName, "_",ss,"_Level4.txt")), showProgress = FALSE)
...
```{r EdUBoxplotsFull, fig.width=12, fig.height=5, eval=TRUE}
...
```{r EdUBoxplotsECMp, fig.width=12, fig.height=5, eval=TRUE}
...
###Explore Selected Signals Level 4 Data


```{r, eval=TRUE}

  #find the curated signals and their values that are in this dataset
 dt <- l4C[,level4Signals$Binding_CP[level4Signals$Binding_CP %in% colnames(l4C)], with=FALSE]
# #Reduce the level4 signals to those that are available
 dtSignals <- level4Signals[level4Signals$Binding_CP %in% colnames(l4C),]
...
fillRow(height = "100%", flex = c(NA, 1), 
        ...
        output$scatterPlot <- renderPlotly({
          ...
          xaxisBinding <- dtSignals$Binding_CP[dtSignals$Name==input$xaxis]
          yaxisBinding <- dtSignals$Binding_CP[dtSignals$Name==input$yaxis]
          colourBinding <- dtSignals$Binding_CP[dtSignals$Name==input$colour]
...
```{r, eval=FALSE}
#find the curated signals and their values that are in this dataset
dtl3 <- l3C[,level4Signals$Binding_CP[level4Signals$Binding_CP %in% colnames(l3C)], with=FALSE]
#Reduce the level4 signals to those that are available
dtSignals <- level4Signals[level4Signals$Binding_CP %in% colnames(l3C),]
...
fillRow(height = "100%", flex = c(NA, 1), 
        verticalLayout(
          ...
          selectInput("yaxis", "Y Axis:", choices = dtSignals$Name, "EdU+ Proportion (normalized)"),
          selectInput("colour", "Colour by:", choices = dtSignals$Name, "Ligand"),
          selectInput("ligands","Filter on Ligands", choices=unique(l3C$Ligand), multiple=TRUE, selectize=TRUE),
          selectInput("ECMps","Filter on ECM Proteins", choices=unique(l3C$ECMp), multiple=TRUE, selectize=TRUE),
          numericInput("SCCThreshold","Spot Cell Count Threshold",value = 0),
          plotOutput("scatterPlot", height = "100%")),
        
        output$scatterPlot <- renderPlotly({
          dtl3 <- dtl3[dt$Spot_PA_SpotCellCount>=input$SCCThreshold,]
          if (!is.null(input$ligands)) dt <- dt[Ligand %in% input$ligands,]
          if (!is.null(input$ECMps)) dt <- dt[ECMp %in% input$ECMps,]
          xaxisBinding <- dtSignals$Binding_CP[dtSignals$Name==input$xaxis]
          yaxisBinding <- dtSignals$Binding_CP[dtSignals$Name==input$yaxis]
          ...
          p <- ggplot(dtl3, aes_string(x=xaxisBinding, y= yaxisBinding, colour=colourBinding))+
            geom_point()+
            labs(x=input$xaxis, y=input$yaxis)
          if (is.null(input$ECMps)|(!length(input$ECMps)==1)) p <- p+stat_smooth(span=1,se=FALSE)
          ggplotly(p)
