...
setkey(dt, Cytoplasm_CP_Intensity_MedianIntensity_MitoTrackerRUV3Loess)
datatable(format(dt[,list(MEP,Barcode, Cytoplasm_CP_Intensity_MedianIntensity_MitoTrackerRUV3Loess,Cytoplasm_CP_Intensity_MedianIntensity_MitoTrackerRUV3Loess_SE,Spot_PA_ReplicateCount)], digits=2, scientific = FALSE, nsmall=0), options = list(pageLength = 5), colnames = c("MEP", "Barcode", "MitoTracker Int.", "MitoTracker Int. SE", "Replicate Count"))

if(!analysisVersion=="v1"){
  setkey(l3HF, Cytoplasm_CP_Intensity_MedianIntensity_MitoTrackerRUV3Loess)
  datatable(format(l3HF[,list(MEP,Barcode,OmeroDetailURL,ArrayRow,ArrayColumn, Cytoplasm_CP_Intensity_MedianIntensity_MitoTrackerRUV3Loess)], digits=2, scientific = FALSE, nsmall=0), options = list(pageLength = 5), colnames = c("MEP", "Barcode","Omero","Row","Column","MitoTracker Int."),escape = FALSE)
}
```
...
```{r ProliferationByMEP,eval=FALSE, fig.width=8, fig.height=6}
...
setkey(dt, Nuclei_PA_Gated_EduPositiveLogitRUV3Loess)
datatable(format(dt[,list(MEP,Barcode,Nuclei_PA_Gated_EduPositiveLogitRUV3Loess,Nuclei_PA_Gated_EduPositiveLogitRUV3Loess_SE,Spot_PA_ReplicateCount)], digits=2, scientific = FALSE, nsmall=0), options = list(pageLength = 5), colnames = c("MEP", "Barcode","EDU+ Prop Norm", "EDU+ Prop Norm SE", "Replicate Count"))

if(!analysisVersion=="v1"){
  setkey(l3HF, Nuclei_PA_Gated_EduPositiveLogitRUV3Loess)
  datatable(format(l3HF[,list(MEP,Barcode,Well,ArrayRow,ArrayColumn,Spot_PA_SpotCellCount,Nuclei_PA_Gated_EduPositiveLogitRUV3Loess, OmeroDetailURL)], digits=2, scientific = FALSE, nsmall=0), options = list(pageLength = 5), colnames = c("MEP", "Barcode","Well","Row","Column","SCC","EDU+ Prop Norm","Omero"),escape = FALSE)
}
```
...
setkey(dt, Cytoplasm_PA_Intensity_LineageRatioLog2RUV3Loess)
datatable(format(dt[,list(MEP,Barcode, Cytoplasm_PA_Intensity_LineageRatioLog2RUV3Loess,Cytoplasm_PA_Intensity_LineageRatioLog2RUV3Loess_SE,Spot_PA_ReplicateCount)], digits=2, scientific = FALSE, nsmall=0), options = list(pageLength = 5), colnames = c("MEP", "Barcode", "Lineage Ratio", "Lineage Ratio SE", "Replicate Count"))

if(!analysisVersion=="v1"){
  setkey(l3HF, Cytoplasm_PA_Intensity_LineageRatioLog2RUV3Loess)
  datatable(format(l3HF[,list(MEP,Barcode,OmeroDetailURL,ArrayRow,ArrayColumn, Cytoplasm_PA_Intensity_LineageRatioRUV3Loess)], digits=2, scientific = FALSE, nsmall=0), options = list(pageLength = 5), colnames = c("MEP", "Barcode","Omero", "Row","Column","LineageRatio"),escape = FALSE)
}
```
...
<br>
```{r SCCRobustZScores, echo=FALSE, fig.width=8, fig.height=3.5, eval=FALSE}
...
The interactive heatmap below is arranged by unsupervised clustering of the rows and columns and colored by the normalized spot cell count. Clicking and dragging across any subsection will zoom in on that section. Double clicking on the zoomed image will return to the full heatmap.  
...
setkey(dt, Spot_PA_SpotCellCountLog2RUV3Loess)
datatable(format(dt[, list(ECMp, Ligand,Barcode,Spot_PA_SpotCellCountLog2RUV3Loess, Spot_PA_SpotCellCount, Spot_PA_ReplicateCount)], digits=1, scientific = FALSE, nsmall=0), options = list(pageLength = 5), colnames = c("ECMp","Ligand","Barcode","Normed Spot Cell Count ","Spot Cell Count","Replicate Count"))
...
PCFVNames <- grep("Norm|RUV3Loess",colnames(l4F),value=TRUE)
