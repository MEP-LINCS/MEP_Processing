...
#Remove the fiducial, blank and control data
setkey(l1F,Ligand)
l1F <- l1F[!"FBS"]
setkey(l1F,ECMp)
l1F <- l1F[!"fiducial"]
...
#Summarize spot level data to MEP level by taking the medians of the parameters
  mepNames<-grep("AreaShape|Children|Intensity|SpotCellCount|Loess|EduPositiveProportion|Population|Neighbors|ReplicateCount|LigandAnnotID|ECMpAnnotID", x=names(l3F),value=TRUE)
...
mDT <- l3F[,list(CellLine,Ligand,Barcode,Endpoint488,Endpoint555,Endpoint647,EndpointDAPI,ECMp),keyby="LigandAnnotID,ECMpAnnotID"]
...
p <- ggplot(l4F[!l4F$EduPositiveProportion==0], aes(x=EduPositiveProportion))+geom_bar(binwidth = .01)+
  #geom_vline(xintercept = c(-2,2), colour = "blue")+
...
p <- ggplot(l4F[!l4F$EduPositiveProportion==0], aes(x = reorder(Ligand, EduPositiveProportion, FUN=median), y = EduPositiveProportion, colour = Barcode))+geom_boxplot()+
  ggtitle(paste("\n\nMEP Proliferation Proportion by Ligand"))+
  xlab("")+ylab("")+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size=rel(.8)), axis.text.y = element_text(angle = 0, vjust = 0.5, hjust=1, size=rel(1)), plot.title = element_text(size = rel(1)),legend.text=element_text(size = rel(.3)),legend.title=element_text(size = rel(.3)))
suppressWarnings(print(p))
```
...
p <- ggplot(l4F, aes(x = reorder(ECMp, EduPositiveProportion, FUN=median), y = EduPositiveProportion))+geom_boxplot()+
  ggtitle(paste("\n\nMEP Proliferation Proportion by ECM Protein"))+
  xlab("")+ylab("")+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size=rel(.8)), axis.text.y = element_text(angle = 0, vjust = 0.5, hjust=1, size=rel(1)), plot.title = element_text(size = rel(1)),legend.text=element_text(size = rel(.3)),legend.title=element_text(size = rel(.3)))
suppressWarnings(print(p))
...
l4QA <- l4QA[!"blank"]
l4QA <- l4QA[!"fiducial"]
l4QA$MEP <- paste(l4QA$ECMp, l4QA$Ligand, sep="-")

#Get numeric features
fv <- grep("_CP_|_PA_|SpotCellCount|MEP$|^Barcode$",colnames(l4QA),value=TRUE)
fv <- grep("Norm|Center", fv, value = TRUE, invert = TRUE)
fvSS2 <- paste("Cytoplasm_CP_Intensity_IntegratedIntensity_Dapi",
                "Cytoplasm_CP_Intensity_IntegratedIntensity_Dapi_MedNorm",
                sep="|")
fv <- grep(fvSS2, fv, value = TRUE, invert = TRUE)
#Create numeric feature vectors datatable
fvDT <- l4QA[,fv,with = FALSE]

activeMEPs <- ubHeatmap(fvDT, title = "Active MEP Feature Vectors", cols = plateCol )
...
#Remove NID1 from the dataset
setkey(fvDT,"MEP")
fvDT <- fvDT[!grepl("NID1", fvDT$MEP)]

activeMEPsF <- ubHeatmap(DT = fvDT, title = "Active MEP Feature Vectors\n (no NID1)", cols = plateCol )
...
activeMEPsFN <- ubHeatmap(DT = fvDT, title = "Active Normalized MEP Feature Vectors\n (no NID1)", cols = plateCol )
...
#Put PCs, MEP and Barcode into a datatable

l4pcvDT <- data.table(l4pcv, MEP = l4fv$MEP, Barcode = l4fv$Barcode)
activeMEPsP <- ubHeatmap(DT = l4pcvDT,title = "PCA of Active MEPS",cols = plateCol)
# 
...
l4pcvDT <- data.table(l4pcv, MEP = l4fv$MEP, Barcode = l4fv$Barcode)
activeMEPsPFN <- ubHeatmap(DT = l4pcvDT,title = "PCA of Active MEPS\n(No NID1)",cols = plateCol)
```
...
l4pcvDT <- data.table(l4pcv, MEP = l4fv$MEP, Barcode = l4fv$Barcode)
activeMEPsPFN <- ubHeatmap(DT = l4pcvDT,title = "PCA of All MEPS",cols = plateCol, activeThresh=0)
```
...
activeEduPPThresh <- quantile(l4F$EduPositiveProportion,probs = .95)
setkey(l4F, EduPositiveProportion)
kable(l4F[l4F$EduPositiveProportion >=activeEduPPThresh, list(Ligand,ECMp,EduPositiveProportion, Nuclei_CP_Intensity_MedianIntensity_Edu_MedNorm)],digits = 2)
```
...
The table below shows the active MEPs (without NID1) based on their euclidean distance from the feature vector of the control wells (High serum wells).

```{r ActiveMepsNoNID, echo = FALSE}
#activeMEPsF <- activeMEPsF[order(activeMEPsF)]
kable(activeMEPsF$MEP, digits = 2)
...
kable(activeMEPsFN$MEP, digits = 2)

```

###Active PCA MEPS
The table below shows the active MEPs based on their PCA vectors and their euclidean distance from the feature vector of the control wells (High serum wells).

```{r ActiveMepsPCA, echo = FALSE}
#activeMEPsP <- activeMEPsP[order(activeMEPsP)]
kable(activeMEPsP$MEP, digits = 2)

```


###Active PCA MEPS
The table below shows the filtered active MEPs based on their PCA vectors and their euclidean distance from the feature vector of the control wells (High serum wells).

```{r ActiveMepsPCAFiltered, echo = FALSE}
#activeMEPsPFN <- activeMEPsPFN[order(activeMEPsPFN)]
kable(activeMEPsPFN$MEP, digits = 2)
