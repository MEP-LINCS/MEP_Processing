...
if (ss == "SS2"){
  ...
  EduPPMedian <- median(l4F$Nuclei_PA_Gated_EduPositiveProportion_RZSNorm, na.rm=TRUE)
  EduPPmad <-mad(l4F$Nuclei_PA_Gated_EduPositiveProportion_RZSNorm, na.rm=TRUE)
  if(EduPPmad == 0) { l4F$EduPP_RZSNorm_RobustZ <- 0
  } else {
    l4F$Nuclei_PA_Gated_EduPositiveProportion_RZSNorm_RobustZ <- (l4F$Nuclei_PA_Gated_EduPositiveProportion_RZSNorm-EduPPMedian)/EduPPmad
  }
}
...
p <- ggplot(l4F[!l4F$Nuclei_PA_Gated_EduPositiveProportion_RZSNorm==0], aes(x=Nuclei_PA_Gated_EduPositiveProportion_RZSNorm))+geom_bar(binwidth = .01)+
  #geom_vline(xintercept = c(-2,2), colour = "blue")+
...
p <- ggplot(l4F[!l4F$Nuclei_PA_Gated_EduPositiveProportion_RZSNorm==0], aes(x = reorder(Ligand, Nuclei_PA_Gated_EduPositiveProportion_RZSNorm, FUN=median), y = Nuclei_PA_Gated_EduPositiveProportion_RZSNorm, colour = Barcode))+geom_boxplot()+
...
p <- ggplot(l4F, aes(x = reorder(ECMp, Nuclei_PA_Gated_EduPositiveProportion_RZSNorm, FUN=median), y = Nuclei_PA_Gated_EduPositiveProportion_RZSNorm))+geom_boxplot()+
...
fvSS3 <- paste("Barcode","MEP",
              ...
              "Nuclei_CP_AreaShape_Eccentricity_RZSNorm",
              "Nuclei_CP_AreaShape_Perimeter_RZSNorm",
              "Nuclei_CP_Intensity_IntegratedIntensity_Dapi_RZSNorm",
              "Nuclei_CP_Intensity_MedianIntensity_Dapi_RZSNorm",
              "Spot_PA_SpotCellCount_RZSNorm",
              "Nuclei_PA_AreaShape_Neighbors_RZSNorm",
              "Nuclei_PA_Cycle_2NProportion_RZSNorm",
              "Nuclei_PA_Cycle_4NProportion_RZSNorm",
...
The next heatmap shows the  analysis run on the dataset after removing NID1 and using data normalized to each plate's control well.
...
#Bug in 2N proportion that is causing infinite values
...
PCFVNames <- grep("Euler|MaximumRadius|Nuclei_PA_Gated_EduPositive_RZSNorm",PCFVNames,invert = TRUE, value = TRUE)
...
p <- ggplot(data.frame(l4PCAModel$x), aes(x = PC1, y = PC2, col = l4QA$Cytoplasm_CP_Intensity_MedianIntensity_Edu_RZSNorm)) +
  geom_point(size = rel(.8), alpha = .8) +
  labs(colour = "Median EdU")+
  theme(legend.text=element_text(size = 6))+
  guides(colour = guide_legend(override.aes = list(size=6)))
print(p)
