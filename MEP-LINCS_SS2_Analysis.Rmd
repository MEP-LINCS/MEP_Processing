...
The proportions are normalized by dividing by the proportion of cells that are Edu+ in the plate's control well  
...
```{r Proliferation, echo=FALSE, fig.width=8, fig.height=3.5, eval=TRUE}

p <- ggplot(l4F[!l4F$Nuclei_PA_Gated_EduPositiveProportion_MedNorm==0], aes(x=Nuclei_PA_Gated_EduPositiveProportion_MedNorm))+geom_bar(binwidth = .01)+
  #geom_vline(xintercept = c(-2,2), colour = "blue")+
...
```{r Proliferation_response, fig.width=8,  fig.height=6, echo=FALSE}

p <- ggplot(l4F[!l4F$Nuclei_PA_Gated_EduPositiveProportion_MedNorm==0], aes(x = reorder(Ligand, Nuclei_PA_Gated_EduPositiveProportion_MedNorm, FUN=median), y = Nuclei_PA_Gated_EduPositiveProportion_MedNorm, colour = Barcode))+geom_boxplot()+
...
p <- ggplot(l4F, aes(x = reorder(ECMp, Nuclei_PA_Gated_EduPositiveProportion_MedNorm, FUN=median), y = Nuclei_PA_Gated_EduPositiveProportion_MedNorm))+geom_boxplot()+
...
fvSS2 <- paste("Barcode","MEP",
               "Nuclei_CP_Intensity_IntegratedIntensity_H3",
...
The next heatmap shows the same analysis run on the entire normalized dataset.
  
```{r, fig.height=16}

#Get the normalized numeric features
fv <- grep("MedNorm|MEP$|^Barcode$",colnames(l4QA),value=TRUE)
fv <- grep("Center_X|Center_Y|SpotNorm", fv, value = TRUE, invert = TRUE)

#Create numeric feature vectors
fvDT <- l4QA[,fv,with = FALSE]

activeMEPsFN <- ubHeatmap(DT = fvDT, title = "All Normalized MEP Feature Vectors", cols = plateCol, activeThresh = 0 )
...
#Get the PCFVs
l4PCAModel <- prcomp(as.formula(paste(" ~ ", paste(grep("MedNorm",colnames(l4QA),value=TRUE), collapse = "+"))), data = l4QA, scale= TRUE)
...
# p <- ggplot(data.frame(l4PCAModel$x), aes(x = PC1, y = PC2, col = l4QA$Cytoplasm_CP_Intensity_IntegratedIntensity_KRT19_MedNorm)) +
#   geom_point(size = rel(.8), alpha = .8) +
...
# 
# 
# p <- ggplot(data.frame(l4PCAModel$x), aes(x = PC2, y = PC3, col = l4QA$Cytoplasm_PA_Intensity_LineageRatio_MedNorm)) +
#   geom_point(size = rel(.8), alpha = .8) +
...
# print(p)
# 
# p <- ggplot(data.frame(l4PCAModel$x), aes(x = PC1, y = PC2, col = l4QA$Cells_CP_AreaShape_Perimeter_MedNorm)) +
#   geom_point(size = rel(.8), alpha = .8) +
...
```{r Content Layout,echo=FALSE, message=FALSE, warnings=FALSE, fig.width=10}
...
```{r Layout Replicate Count,echo=FALSE, message=FALSE, warnings=FALSE, fig.width=6.5, fig.height=6}
...
```{r FilteredReplicateCount,echo=FALSE, message=FALSE, warnings=FALSE, fig.width=6.5, fig.height=6}
...
setkey(l4F, Spot_PA_SpotCellCount_MedNorm_RobustZ)
kable(l4F[l4F$Spot_PA_SpotCellCount_MedNorm_RobustZ >=2| 
            l4F$Spot_PA_SpotCellCount_MedNorm_RobustZ <=-2, list(Ligand,ECMp,Spot_PA_SpotCellCount_MedNorm_RobustZ, Spot_PA_SpotCellCount)],digits = 2)
```

\newpage

###Extreme Proliferation MEPs

```{r ProliferationMeps,echo=FALSE, fig.width=8, fig.height=5, eval=TRUE}

activeEduPPThresh <- quantile(l4F$Nuclei_PA_Gated_EduPositiveProportion,probs = .95)
setkey(l4F, Nuclei_PA_Gated_EduPositiveProportion)
kable(l4F[l4F$Nuclei_PA_Gated_EduPositiveProportion >=activeEduPPThresh, list(Ligand,ECMp,Nuclei_PA_Gated_EduPositiveProportion, Nuclei_CP_Intensity_MedianIntensity_Edu_MedNorm)],digits = 2)
```

\newpage

###Active MEPS from Unbiased Analysis
The table below shows the active MEPs based on their euclidean distance from the feature vector of the control wells (High serum wells).

```{r ActiveMeps, echo = FALSE}
kable(activeMEPs$MEP, digits = 2)
...
if (ss == "SS1"){
  ...
  mDT <- l1[l1$Barcode == barcode]
  mDT <- mDT[mDT$Nuclei_CP_Intensity_IntegratedIntensity_Dapi > quantile(mDT$Nuclei_CP_Intensity_IntegratedIntensity_Dapi, probs=.01, na.rm=TRUE) & mDT$Nuclei_CP_Intensity_IntegratedIntensity_Dapi < quantile(mDT$Nuclei_CP_Intensity_IntegratedIntensity_Dapi,probs=.98, na.rm=TRUE)]
  p <- ggplot(mDT, aes(x=Nuclei_CP_Intensity_IntegratedIntensity_Dapi, y = log2(Nuclei_CP_Intensity_MedianIntensity_Edu), colour = Nuclei_PA_Gated_EduPositive))+geom_point(size = rel(.6), alpha = .8)+
    facet_wrap(~Ligand, nrow=2, scales="free_x")+
    ggtitle(paste("\n\n","Edu vs. Total DAPI Signal,",barcode))+
    ylab("Edu (log2)")+xlab("Total Intensity DAPI")+
    theme(strip.text = element_text(size = 5))
  suppressWarnings(print(p))
}
} 
...
MEP <- unique(l2[,list(ECMp,Ligand,MitoPerimeterRatio,MitoSparseRatio,Barcode,Well,ArrayRow,ArrayColumn,Spot_PA_SpotCellCount,MeanIntensityAlexa647SpotNorm)])
